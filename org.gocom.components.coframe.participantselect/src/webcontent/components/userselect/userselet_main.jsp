...
<div class="mini-toolbar" style="padding:2px;border-top:0;border-left:0;border-right:0;">                
	...
	<a id="recent" class="mini-button" checkOnClick="true" groupName="select" iconCls="icon-function-group-open" plain="false" onclick="switchTab('recent')">最新访问</a>
	<% } %>
	<% if("true".equals(request.getParameter("deptSelect"))){ if(firstKey.equals("")) firstKey= "dept" ;%>
	<a id="dept" class="mini-button" checkOnClick="true" groupName="select" iconCls="icon-organization" plain="false" onclick="switchTab('dept')">机构</a>     
	  <% } %>          
	   <% if("true".equals(request.getParameter("roleSelect"))){ if(firstKey.equals("")) firstKey= "role" ;%>
	<a id="role" class="mini-button" checkOnClick="true" groupName="select" iconCls="icon-role" plain="false" onclick="switchTab('role')">角色</a>  
	 <% } %>
	 <% if("true".equals(request.getParameter("groupSelect"))){ if(firstKey.equals("")) firstKey= "group" ;%>    
	<a id="group" class="mini-button" checkOnClick="true" groupName="select" iconCls="icon-group" plain="false" onclick="switchTab('group')">工作组</a>  
	 <% } %>
	 <% if("true".equals(request.getParameter("onlineSelect"))){ if(firstKey.equals("")) firstKey= "online" ;%>
	<a id="online" class="mini-button" checkOnClick="true" groupName="select" iconCls="icon-duty-manager" plain="false" onclick="switchTab('online')">在线用户</a>     
	 <% } %> 
	 <% if("true".equals(request.getParameter("searchSelect"))){ if(firstKey.equals("")) firstKey= "search" ;%>
	<a id="search" class="mini-button" checkOnClick="true" groupName="select" iconCls="icon-search" plain="false" onclick="switchTab('search')">查询</a>     
	 <% } %> 
</div>
	...
	<div class="mini-splitter" style="width:100%;height:100%;">
		...
		<div showCollapseButton="true">
			<div class="mini-toolbar"
				...
				borderStyle="border:0;">
				<a class="mini-button" iconCls="icon-add" plain="false"
					onclick="selectAll()">
					全部选中
				</a>
				<a class="mini-button" iconCls="icon-remove"
					plain="false" onclick="removeAll()">
					全部删除
				</a>
			</div>
			<div class="mini-fit">
