                ...
                var json1 = nui.encode({attachs:data});
                	nui.ajax({
						...
						success:function(text){
		                    if(text.flag==false){
		                       window.parent.nui.alert("下载失败，文件不存在！", "系统提示");
		                    }else{
		                        //请求下载页面流
    	...
    	if (rows.length > 0) {
    		for (var i = 0; i < rows.length; i++) {
    			...
    			if(row.vcUserId!='<%=userCode %>'){
    				window.parent.nui.alert("不能删除他人上传的文件");
    				return;
    			}
    			ids += row.lAttachId;
    			if(i<rows.length - 1){
		...
		$.ajax({
            ...
            success:function(data){
	             ...
	             if(returnJson.returnValue == "ok"){
	               window.parent.nui.alert("附件删除成功！","系统提示",function(action){
	                   removeRow();
	                   window.parent.refreshFile();
	                   //window.parent.loademployeeGrid();
	               });
	             }else{
	               window.parent.nui.alert("附件删除失败！","系统提示",function(action){});
	             }
        	 }
         });
	        }
