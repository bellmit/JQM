...
<%
	...
	DiskFileItemFactory factory = new DiskFileItemFactory();
		...
		if(attachType.equals("1")){
			...
			if(attachBusType.equals("12")){
				base = ProductUtil.UPLOAD_FILE_PATH_PRODUCTINFO_SEAL;
			}
		}
		...
		//逻辑构建名称
			...
			//逻辑流名称
			String operationName = "saveAttachmentInfo";
		...
		try {
			...
			for (int i = 0 ;i < items.size(); i++){
				...
				if (!item.isFormField() && attachName.length() > 0) {
			    	...
			    	fileName = base + File.separator + lProductCode + "_" + DateUtil.currentDateString(DateUtil.YYYYMMDDHHMMSS_NUMBER) + "_" + attachName.substring(0,k) +'.'+extention;
			    	} 
					...
					Object[] params = new Object[]{attachName,fileName,  item.getSize(),usreId,bizId,attachType,attachBusType,workItemID,processinstid,lProductId};
