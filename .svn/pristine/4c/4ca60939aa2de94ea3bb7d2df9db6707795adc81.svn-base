	...
	<script type="text/javascript">
    	...
    	//tab页权限处理
		 nui.ajax({
            ...
            type:'POST',
            data:{funcCode:"ATS_ZHXXCX_GZB,ATS_ZHXXCX_YYFJ,ATS_ZHXXCX_JZB,ATS_ZHXXCX_KMYEB"},
            cache:false,
            ...
            success:function(text){
	        	...
	        	if(result){
	        		...
	        		for(var k=0;k<result.length;k++){
	        			if(result[k].RES_ID=="ATS_ZHXXCX_GZB"){
							tempArray[0]=result[k];
						}
						if(result[k].RES_ID=="ATS_ZHXXCX_YYFJ"){
							tempArray[1]=result[k];
						}
						if(result[k].RES_ID=="ATS_ZHXXCX_JZB"){
							tempArray[2]=result[k];
						}
						if(result[k].RES_ID=="ATS_ZHXXCX_KMYEB"){
