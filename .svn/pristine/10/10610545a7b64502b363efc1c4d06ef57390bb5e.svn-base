	...
	<script type="text/javascript">
    	...
    	var queryUserType = "<%=request.getParameter("queryUserType")%>";
		 nui.ajax({
            ...
            type:'POST',
            data:{funcCode:"ATS_ZHXXCX_CCXX,ATS_ZHXXCX_TZZCJFZXX,ATS_ZHXXCX_ZGJHDCZCXX"},
            cache:false,
            ...
            success:function(text){
	        	...
	        	if(result){
	        		...
	        		for(var k=0;k<result.length;k++){
	        			if(result[k].RES_ID=="ATS_ZHXXCX_CCXX"){
							tempArray[0]=result[k];
						}
						if(result[k].RES_ID=="ATS_ZHXXCX_TZZCJFZXX"){
							tempArray[1]=result[k];
						}
						if(result[k].RES_ID=="ATS_ZHXXCX_ZGJHDCZCXX"){
