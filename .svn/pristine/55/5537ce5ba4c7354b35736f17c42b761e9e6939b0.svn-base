...
<div style="margin:0px 2px 0px 2px;width:100%;height:100%;" >
		   ...
		   <div class="search-condition">
			   <div class="list">
				 <div id="form_ZQMM" class="nui-form" style="padding:2px;" align="center" style="height:10%">
	                ...
	                <!-- 获取01-债券买入、02-债券卖出类型；业务类别（分六类：1.债券买卖;2.正逆回购;3.申购缴款;4.基金申赎;5.债券买卖;6.追加提取;）：01-债券买入;02-债券卖出;03-质押式正回购;04-质押式逆回购;05-买断式正回购;06-买断式逆回购;07-债券申购;08-债券缴款;09-基金申购;10-基金赎回;11-基金转换;12-同存存入;13-同存提取;14-资金追加;15-资金提取;99-其他指令/建议; -->
	...
	<script type="text/javascript">
	    ...
	    grid_ZQMM.on("rowdblclick", function (e) {
	    	rowdblclickFun_common_ZQMM("JY_ZQMM/CFJYProductProcessForm_show_ZQMM.jsp", "债券买卖明细展示", 750, 510, e, grid_ZQMM, "query", "TG");
	    });
	    
	    //编辑
	    function edit_ZQMM() {
	    	edit_common_ZQMM("JY_ZQMM/CFJYProductProcessForm_ZQMM.jsp", "债券买卖编辑数据", 750, 510, grid_ZQMM, "edit", "TG");
	    }
	    //废弃
	    ...
	    //新增
	    function add_ZQMM() {
	    	add("JY_ZQMM/CFJYProductProcessForm_ZQMM.jsp", "债券买卖新增记录", 750, 510, grid_ZQMM);
	    }
	...
	function confirm(grid_confirm){
	    ...
	    if(rows.length > 0){
    		...
    		for(var i=0; i<rows.length; i++){
	        	...
	        	//投顾只能确认“交易员填单指令/建议补齐完成”或“投资顾问录入完成”阶段状态的指令
    ...
    //全屏显示
	function fullScreen(){
		window.open('<%=request.getContextPath()%>/ProductProcess/JY_ZQMM/CFJYProductProcessList_ZQMM.jsp','newwindow','height=100,width=400,top=0,left=0,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
	}
