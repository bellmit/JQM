		...
		if(sendType!=null&&!"".equals(sendType)){  //判断非空
			...
			for(int i = 0; i < mailTypes.length; i++){
				...
				if(mailType.indexOf("Mail")>-1){			 //判断发送类型是否邮件
					...
					//设置邮件内容
					String mailContext = MailManager.sendMailContext(mailType, prodName, currentActivityName,nextActivityName);
					System.out.println("邮件内容："+mailContext);
					System.out.println("邮件接收人："+MailTo);
					//发送邮件
					try {
						sendFlag = MailManager.sendHtmlMail(mailTitle, mailContext, MailTo, MailCC, AttachFilePath, AttachFileName);
