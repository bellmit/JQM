...
function riskControlTrial() {
    var param = combiDireParam(false);
    if (!param) {
        return false;
    }
    ...
    nui.ajax({
        ...
        success: function (resp) {
            if(resp.avaiList.length>0){
            	...
            	nui.get("vcAvailableamountT0").setValue(formatNumber(vcAvailableamountT0+updateAmont,2,1));
                nui.get("vcAvailableamountT1").setValue(formatNumber(vcAvailableamountT1+updateAmont,2,1));
            }
        },
        //有错误码之后，把后面的错误提醒补齐。
        fail: function (resp) {
    ...
    nui.ajax({
        ...
        success: function (resp) {
            if(resp.enFundValue.length>0){
                nui.get("enFundValue").setValue(resp.enFundValue[0].enFundValue);//基金净资产(元)
            }
        },
        //有错误码之后，把后面的错误提醒补齐。
        fail: function (resp) {
...
function calEmFaceAmount() {
	...
	var value = nui.get("enRepoRate").getValue();//回购利率
	...
	//计算中途付息利率(最后付息日，付息频率，券面金额，券面利率，首期结算日期，券面利率，到期结算日期）
	...
	//回购利率，首次全价，  占款天数，最后付息日 ， 到期结算日期，付息频率，券面利率期，中途付息利率
...
//            	row["enFullPriceInitA"] = Number(resultArr[1]);//全价（元/百元面值）没有四舍五入的值
            	...
            	//计算可回购金额    ((全价（元/百元面值）*( 券面金额/100))*10000)保留小数点后两位
...
//净价 ，到期全價 ，grid，grid第一行，回调函数
function calcenFullPriceFinal(netValue,allValue,grid,row,recall){
	if(!netValue && !allValue){
		return;
	}
	var lTradeDate = nui.get("lMaturitySettleDate").getValue();//到期结算日期
	if(!lTradeDate){
		nui.alert("请输入回购天数！");
		return;
	}
	...
	//计算中途付息金额     （下期付息日期，付息利率，券面金额，券面利率,首次结算日期）
	var middleInterest = calMiddleInterest(row["lNextPayintDate"],row["enPayInteval"],row["enFaceAmount"],row["enFaceRate"],nui.get("lFirstSettleDate").getValue(),
			nui.get("lMaturitySettleDate").getValue());
	nui.get("enMiddleInterest").setValue(mul(middleInterest,row["enFaceAmount"] * 100));//中途付息金额*券面金额*100
	
	
	var jsonParam = {processDate:lTradeDate, ////到期结算日期
			endDate:DateUtil.toNumStr(nui.get("lMaturitySettleDate").getValue()),//到期结算日期
			interCode:row['vcInterCode'],reportCode:row["vcStockCode"],
			netPrice:netValue, fullPrice:allValue, interestRate:null, interestRateType:'1', 
			tradingPlace:'5', clearingSpeed:vcSettleSpeed,'payment':payment};/*payment：中途付息金额*/
	nui.ajax({
...
//            	row["enFullPriceFinalA"] = Number(resultArr[1]);//全价（元/百元面值）没有四舍五入的值
				grid.updateRow(row);
				//到期结算金额(元)  = （ 全价（元/百元面值）*（ 券面金额（万元）/100））* 10000，保留小数点后两位
				...
				//计算回购利息=(到期结算金额(元)-首期结算金额)+(中途付息金额*(券面金额（万元）*100))
...
function calInterest(allValueInit,allValueFinal,period,lLastPayintDate,endDate,enPayInteval,enFaceRate,tc){
    ...
    if(lLastPayintDateNum && (Number(DateUtil.toNumStr(endDate)) > Number(lLastPayintDateNum))){ //此条记录是否存在下一付息日同时下一付息日是否小于到期结算日
		...
		var dTime = DateUtil.DstrToDate(endDate).getTime() - lLastPayintDate.getTime();//到期结算日-下一付息日期
		var d = dTime/86400000;
		var son = add((allValueFinal - allValueInit),TC);//（（到期全价-首期全价）+中途付息金额和数量）
		var IPD = div(mul(allValueInit,period),365);//（首期全价*实际占款天数）/365）
		var TCd = div(mul(TC,d),365);//（（到期结算日-下一付息日期）*（中途付息金额和数量））/365
		interest = div(son,(IPD - TCd)) * 100;//（（（（到期全价-首期全价）+中途付息金额和数量）除以（（（首期全价*实际占款天数）/365）-（（到期结算日-下一付息日期）*（中途付息金额和数量））/365）））*100
	}else{
		var FPIP = (allValueFinal - allValueInit);//到期全价-首期全价
		var IPD = mul(allValueInit,period);//首期全价*实际占款天数
		interest = div(mul(FPIP,365),IPD) * 100;//（（（到期全价-首期全价）*365）/（首期全价*实际占款天数））*100
	}
...
function calNetFull(enRepoRate,enFullPriceInit,period,lLastPayintDate,endDate,enPayInteval,enFaceRate,tc){
	...
	var lLastPayintDateNum = DateUtil.toNumStr(lLastPayintDate);//最后付息日
	if(lLastPayintDateNum && (Number(DateUtil.toNumStr(endDate/*到期结算日期*/)) > Number(lLastPayintDateNum))){ //此条记录是否存在下一付息日同时下一付息日是否小于到期结算日
		var TC = tc;//入参改为了中途付息金额和数量//中途付息利率
		var dTime = DateUtil.DstrToDate(endDate).getTime() - lLastPayintDate.getTime();//到期结算日期-最后付息日
		var d = dTime/86400000;
		var RIPD = mul(mul(enRepoRate/100,enFullPriceInit),period);//（（（回购利率/100）*首次全价）* 占款天数）
		var RTCD = mul(mul(enRepoRate/100,TC),d);//（（（回购利率/100）*中途付息利率））*（到期结算日期-最后付息日）
		var son = RIPD - RTCD;		//（（（回购利率/100）*首次全价）* 占款天数）-（（（回购利率/100）*中途付息利率））*（到期结算日期-最后付息日）
		enFullPriceFinal = add(enFullPriceInit,div(son,365)) - TC;//（首次全价+（（（（（回购利率/100）*首次全价）* 占款天数）-（（（回购利率/100）*中途付息利率））*（到期结算日期-最后付息日）））/365）
	}else{
		var RIP = mul(enRepoRate/100,enFullPriceInit);//（回购利率/100）*首次全价
		var D365 = div(period,365);//占款天数/365
		var IPDR365 = mul(RIP,D365);//（（（回购利率/100）*首次全价）/（占款天数/365））
		enFullPriceFinal = add(IPDR365,enFullPriceInit); //得到到期全價       （首次全价+（（回购利率/100）*首次全价）/（占款天数/365）））
	}
...
//组装完整指令参数
function combiDireParam(isRisk){
    ...
    if(fullDirection){
    	 ...
    	 if(!fullDirection.isChanged() && (!isbondsChange) && isRisk){
...
//    fullDire["enFullPriceAmount"] = 0 + parseInt(fullDire["enFaceAmount"]);
    ...
    directionParam["businClass"] = "O";  //	2 (业务类别:2-银行间二级市场业务 A –开放式基金B-存款业务C-网下申购D-债券一级市场)
...
function sendDirective() {
    ...
    //提交动作。
    var url = "com.cjhxfund.ats.sm.comm.InstructionManager.checkInstructAvail.biz.ext";
        ...
        nui.ajax({
            ...
            success: function (e) {
                ...
                if(returnJson.exception == null){
                	if(returnJson.rtnCode == ATS_SUCCESS){
            		...
            		}else if(returnJson.rtnCode == "203" || returnJson.rtnCode == ATS_ORDER_USABLE_AMOUNT_INSUFFICIENT){//可用不足,继续下单先校验风控
            			...
            			var riskMsg = returnJson.riskMsg;
            			if(riskMsg && riskMsg.isAgainstRisk){//触发了风控
