...
<!-- 	            <input class="nui-hidden" name="bizCriteria/_orderby[2]/_property" value="dPaymentDate" /> -->
                ...
                <table id="table1" border="0" cellpadding="1" cellspacing="1" style="width:100%;table-layout:fixed;">
                    <tr>
                        ...
                        <td style="width:15%" align="left">
                            <input class="nui-datepicker" id="dPaymentDate" format="yyyy-MM-dd" name="bizCriteria/_and[1]/_or[1]/_expr[1]/dPaymentDate" width="100%"/>
                    ...
                    <tr>
                        ...
                        <td align="left" style="width:80px">
                        	<input id="vcProcessType" class="nui-hidden" name="bizCriteria/_expr[11]/vcProcessType"   width="100%" />
                        	<input class="nui-hidden" name="bizCriteria/_expr[11]/_op" value="in">
					        <div id="debt" name="today" class="mini-checkbox" readOnly="false" value="true" text="申购"></div>
							&nbsp;&nbsp;
							<div id="payment" name="productId" class="mini-checkbox" value="true" readOnly="false" text="缴款" ></div>  
							&nbsp;&nbsp;
							<!-- 使用缴款面值来判别查询的数据，如果勾选了申购，仅查询缴款面值为空的申购指令/建议，如果勾选了缴款这查询缴款流程和申购指令/建议缴款面值不为空的数据 -->
							<input class="nui-hidden" id="enPayFaceValue" name="bizCriteria/_expr[16]/enPayFaceValue" width="100%"/>
                ...
                <table style="width:100%;">
                    <tr>
                        <td style="width:100%;">                            
                            ...
                            <privilege:operation sourceId="ATS_SII_SIID" sid="stockIssueInfoDown" clazz="nui-button" onClick="wordDownload1()" lableName="新债信息表下载"  iconCls="icon-download" ></privilege:operation>
		 ...
		 流程ID
                        ...
                        <div field="vcStockCode" headerAlign="center" align="left" allowSort="true" >
                            债券代码
                        </div>
                        <div field="vcStockName" headerAlign="center" allowSort="true" >
                            债券简称
                        </div>
                        <div field="vcStocktypeName" headerAlign="center" allowSort="true" width="70px" align="center" renderer="renderCStockType">
                            债券类别
                        </div>
                        <div field="enExistLimite" headerAlign="center" allowSort="true" width="80px" align="right">
                           发行期限(年)
                        </div>
                        <div field="enIssueBalance" headerAlign="center" align="right" allowSort="true" width="80px">
                            发行规模(亿)
                        </div>
                        <div field="cIssueAppraise" headerAlign="center" align="center" allowSort="true" width="60px" renderer="renderCIssueAppraise">
                            主体评级
                        </div>
                        <div field="vcPaymentPlace" headerAlign="center" allowSort="true" width="100px"renderer="onvcPaymentPlace">
                            登记托管机构
                        </div>
                        ...
                        <div field="vcIssueProperty" headerAlign="center" align="center" allowSort="true" width="100px" renderer="renderVcIssueProperty">
                            主体类型
                        </div>
                        
                        <div field="lInvestNo" headerAlign="center" align="center" allowSort="true" width="60px">
                            投资编号
                        </div>
	 	...
	 	grid.on("drawcell", function (e) {
           ...
           function search() {
			  ...
			  var startTime2=Date.parse(new Date(processDate.replace(/-/g,"/")));
           	  ...
           	  var debt=nui.get("debt").getValue();
           	  var payment=nui.get("payment").getValue();
           	  if(debt=="false" && payment=="false"){
           	  	nui.alert("必须选择一个流程类型");
           	  	return;
           	  }
           	  debt=(debt=="false"?"0":"1,5");
           	  payment=(payment=="false"?"0":"8");
           	  
           	  if(debt!="0" && payment=="0"){
           	    nui.get("enPayFaceValue").setValue("-1");
           	    nui.get("enPayFaceValueOp").setValue("=");
           	  }
           	  //console.log(debt+"=="+payment);
           	  if(debt=="0" && payment!="0"){
           	  	nui.get("enPayFaceValue").setValue("0");
           	    nui.get("enPayFaceValueOp").setValue(">=");
           	    debt="1,5";
           	  }else if(debt!="0" && payment!="0"){
           	  	nui.get("enPayFaceValue").setValue("");
           	    nui.get("enPayFaceValueOp").setValue("");
           	  }
           	  
           	  nui.get("vcProcessType").setValue(debt+","+payment);
            ...
            function reset(){
		     ...
		     function selectionChanged(){
		        ...
		        if(rows.length>1){
		               ...
		               //nui.get("placeOrder").disable();
		               nui.get("stockIssueInfoDown").disable();
		        }else{
		        	   nui.get("revisedOffer").enable();
		               nui.get("revokeOffer").enable();
		               nui.get("addAll").enable();
		               nui.get("flowChart").enable();
		               //nui.get("placeOrder").enable();
		               nui.get("stockIssueInfoDown").enable();
		...
		if(row){
			...
			if(vcProcessType=="1" || vcProcessType=="5"){
				...
				nui.open({
						...
						title: "缴款流程修改报价", width: 750, height: 520,
						onload: function () {
							//弹出页面加载完成
							var iframe = this.getIFrameEl();
							var data = {pageType:"add",processInstID:processInstID,bizId:bizId};//传入页面的json数据
			...
			nui.confirm("是否确认要撤销报价？","系统提示",function (a){
				if(a!="ok"){ return; }
	                if(vcProcessType=="1" || vcProcessType=="5"){
						...
						var activity=["ATS_FM_XYYY2","ATS_FM_XYYY1","ATS_FM_SCSFYYXY","ATS_FM_ZJQS",
						              "ATS_FM_PDFXYYY","ATS_FM_FXYYYFH","ATS_FM_FXYYY"];
						for(var i=0;i<activity.length;i++ ){
						  if(activityDefID==activity[i]){
						     nui.alert("交易员协议用印判定之后，无法撤销报价！");
							 return;
						  }
						}
					}
		...
		$.ajax({
			...
			success:function(text){
				...
				if(returnJson.isSuccess == "4"){
					nui.alert(returnJson.failReason == null ? returnJson.text : returnJson.failReason,"系统提示");
		        ...
		        if(text.participantId.indexOf(userId_zbqr) < 0){
		           ...
		           if(text.isInvestment==1 && text.isProduct>0){
			        ...
			        /* else if(text.isInvestment==0 && text.isProduct==0){
			          nui.alert("当前登录用户不能操作非招行产品，不能中标确认！");
			          bool=false;
			        } */
			        else{
