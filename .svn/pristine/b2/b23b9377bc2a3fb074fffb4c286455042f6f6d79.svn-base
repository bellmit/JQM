...
<body style="width: 100%;height:99%;overflow: hidden" class="body-font" onload="isshowAccountType()"> 
	...
	<div id="taskListId" class="nui-datagrid" style=" width: 98%;height: 88%;display: none"  showPager="true"
		url="com.cjhxfund.fpm.bpsExpend.task.queryViewedNoticeList.biz.ext" allowAlternating="true" sortField="arrivalTime" sortOrder="desc"
	    ...
	    <div property="columns">
			...
			<div name="dealDevice" field="dealDevice" width="15%" headerAlign="center" allowSort="true" align="center">状态</div>
			<div name="creator" field="creator" width="15%" headerAlign="center" allowSort="true" align="center">提交人</div>
		    <div name="arrivalTime" field="arrivalTime" width="15%" headerAlign="center" dateFormat="yyyy-MM-dd HH:mm:ss" align="center" allowSort="true">到达时间</div>
	    </div>
	</div>
	...
	<script type="text/javascript">
    	...
    	function isshowAccountType(){
	    	...
	    	taskListDataGridObj.on("update", function() {	
	    		window.parent.refrushTaskNum("viewedNotice",taskListDataGridObj.totalCount);
			});
    	}
        //加载任务列表
    	...
    	function outputTaskName(e){
            ...
            var	s = '<a class="dgBtn"  style = "font-size: 9pt;" href="javascript:doOperate(' + e.row.taskId + ',\''+e.row.taskType+'\','+e.row.processInstID+',\'' + taskName + '\')">'+taskShowName+'</a> ';
            return s; 		
    	}

	    //执行操作
	    function doOperate(taskId,taskType,processInstID,title) {
	    	var url = "";
	    	...
	    	if(status){
		    	...
		    	} else if (taskType == "ActivityInst"){
		    		openPrintPage(processInstID,title);
		    	}
		    }else{
		         //alert(taskType);
		      if(taskType == "task"){
	    		...
	    		openPrintPageOld(processInstID,title);
	    	}
		...
		function confirmNotice(notificationID,processInstID,title) {			
                        ...
                        openTab("viewedInformation" + processInstID, "tab$viewedInformation$" + processInstID, title, url);					
					}
