...
<%
  ...
  String mServerName="commonUtil/JQMHistory/iWebOffice/OfficeServer.jsp";
  ...
  if (DbaObj.OpenConnection()){
    ...
    try{
      ...
      if (result.next()){
        ...
        mFileName = result.getString("attach_name");
...
function Load(){
  try{
    ...
    webform.WebOffice.WebUrl="<%=mServerUrl%>?Filepath=<%=URLEncoder.encode(FileAdd,"utf-8") %>";      //WebUrl:系统服务器路径，与服务器文件交互操作，如保存、打开文档，重要文件
...
<body bgcolor="#ffffff" onload="Load()" onunload="UnLoad()">  <!--引导和退出iWebOffice-->
  <form name="webform" method="post" action="iwebPDF/DocumentSave.jsp"  onsubmit="return SaveDocument();">  <!--保存iWebOffice后提交表单信息-->
    ...
    <table border=0 cellspacing='0' cellpadding='0' width=100% height=100% align=center class=TBStyle >
     <%-- <tr>
      ...
      <tr width="100">
        <td align="center" valign=top class="TDTitleStyle" width="100" height="100%">
