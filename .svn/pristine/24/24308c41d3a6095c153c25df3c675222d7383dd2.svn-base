...
- Description:
    ...
    <div id="dataform1">
		<table style="width:100%; table-layout:fixed;margin: 0xp;padding: 10px;" id="table" border="0" class="">	
			<tr>
                <td style="width:100px; " align="right">标签名称：</td>
                <td >
                	<input id="vcLabelName" name="vcLabelName" class="mini-textbox" style="width:80%"  />
                </td>
             </tr>
          </table>
	</div>
     <div class="nui-toolbar" id="submit_button" style="padding:30px 0 0 0;" borderStyle="border:0;">
        ...
        <script type="text/javascript">
        	...
        	function SetData(data){
        		...
        		label={"vcLabelSeq":data.vcLabelSeq,"lParentId":data.nodeId};
        	}
        	function add(){
        		label.vcLabelName=$.trim(nui.get("vcLabelName").getValue());
        		
        		if(label.vcLabelName==""){
        			nui.alert("请输入标签名称~");
        			return;
        		}
        		 nui.ajax({
		            url: "com.cjhxfund.jy.ProductProcess.o32tcjc.addLabel.biz.ext",
		            type: 'POST',
		            data: {label:label},
		            cache: false,
