...
<html>
<!-- 
  - Author(s): 分组管理，该界面 由group_tree.jsp 完全复制而来
...
<div id="layout1" class="nui-layout" style="width:100%;height:92%;">
	<div title="分组管理" region="west" bodyStyle="overflow:hidden;" width="240" class="sub-sidebar" allowResize="true">
...
<script type="text/javascript">
	...
	$(function() {
	    ...
	    var rootNode = {nodeName: "分组管理", nodeType: "Root", isLeaf: false, expanded: false, iconCls: "icon-keywords"};
    ...
    function onBeforeTreeLoad(e) {
    	e.url = "com.cjhxfund.jy.ProductProcess.o32tcjc.queryGroupTreeChildNodes.biz.ext";
	...
	// 根结点对应的Tab页
	var employee_index = { title: '产品信息', url: '<%=contextPath%>/ProductProcess/zhxxcx/o32tcjc1_new/group_list.jsp' };
	
	// 机构结点对应的Tab页 
	var org_add = { title: '增加分组', url: '<%=contextPath%>/ProductProcess/zhxxcx/o32tcjc1_new/add_prod_group.jsp' };
	...
	// 机构右键菜单
	var prodMenu = [
		{text: "增加分组", onclick: "onAddOrgOfOrg", iconCls: "icon-add"},
		{text: "删除分组", onclick: "executeOrgRemove", iconCls: "icon-remove"},
		{text: "刷新", onclick: "onRefreshNode", iconCls: "icon-reload"}
	];
	...
	function searchParentOrgNode(node) {
		...
		if(node.nodeType == "ProductTree" || node.nodeType == "Root" ) return node;
...
//		}
	...
	function onAddOrgOfOrg(e) {
		...
		if(getSelectedNode().nodeType!="Root" && getSelectedNode().lParentId!="-1"){
	...
	function executeOrgRemove(){
	   ...
	   nui.confirm("该操作将删除分组和分组关联产品信息，是否确认删除？","系统提醒",function(action){
			if(action=="ok"){
			    var tree = nui.get("tree");
				var node = tree.getSelectedNode();
				var parentNode = tree.getParentNode(node);
				...
				var data = {
					lGroupId: node.nodeId
				};
				
				var json = nui.encode(data);
		        nui.ajax({
		            url: "com.cjhxfund.jy.ProductProcess.o32tcjc.deleteGroup.biz.ext",
		            type: 'POST',
		            data: json,
		            cache: false,
		            contentType:'text/json',
