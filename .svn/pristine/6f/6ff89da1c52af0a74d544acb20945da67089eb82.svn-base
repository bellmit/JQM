    ...
    <div size="30%" showCollapseButton="true">
         <div id="deptsearch" class="mini-fit" style="height:100%;" borderStyle="border:0;">
            ...
            <ul id="depttree" class="nui-tree" url="com.cjhxfund.fpm.bpsExpend.empOrg.findAllorganization.biz.ext" style="width:100%;height:100%;"
                showTreeIcon="true" textField="orgname" idField="orgid" parentField="parentorgid" dataField="organizations" resultAsTree="false"
                showExpandButtons="true" onload="onload" expandOnLoad="true">        
            </ul>
        </div>
      </div>
    <div showCollapseButton="true">
        <div id="form1" class="nui-form" align="center" style="height:100%;height:15%;">
         ...
         <div class="nui-fit" style="width:100%;height:85%;">
            <div <%-- id="usergrid1" class="nui-datagrid" style="width:100%;height:100%;"
                url="com.cjhxfund.fpm.util.userselect.deptUserList.biz.ext"
               ...
               id="usergrid1"
                ...
                <div property="columns">
                   ...
                   <div field="orgname" width="110" headerAlign="center"  align="center" allowSort="true">所属机构</div>
                   <div field="userselect" width="30" headerAlign="center"  align="center"  renderer="showcheckbox">
                   		<input id="selectAllChk" type="checkbox" onclick="revSelect(this)"/></div>                                   
                </div>
            </div>  
        </div>
    </div>        
...
<input id="selectUsers" class="mini-textboxlist" allowInput="false" name="selectUsers" textName="selectUsersName" required="true" style="width:100%;height:50px;"
        value="" text="" 
        valueField="empid" textField="text" 
/>
</div>
</div>
<div class="mini-toolbar" style="text-align:center;padding-top:8px;padding-bottom:8px;" borderStyle="border:0;">
        ...
        <!-- <a class="mini-button" iconCls="icon-cancel" style="" onclick="closeWindow('clean')">清除</a>
        <span style="display:inline-block;width:25px;"></span> -->
...
<script type="text/javascript">
	...
	function onrowclick(e){
		...
		if(chkstr==""){
			$("#chk_" + record.targetId).trigger("click");
		}
		if(checked){
	...
	function showcheckbox(e){
		...
		return '<input id="chk_'+record.targetId+'" type="checkbox" targetId="'+record.targetId+'" targetName="'+record.targetName
			...
			+  '" name="useselect" ' + checkstr + ' class="selcheckbox" onclick="chk();">';
	
	}
	
	//表格加载完成后触发，修改全选按钮方法
	grid.on("update", changeSelectAllChk);
	
	//设置默认全选选中
	function changeSelectAllChk(){
		var selectAllChk =$("#selectAllChk");
		var selectFlag = true;
		var	records=grid.getData();
		for(var i=0; i<records.length;i++){
			var checkObj = $("#chk_" + records[i].targetId);
			 if(!checkObj.attr("checked")){//有未选中的
			 	selectFlag = false;
			 }
		} 
		if(selectFlag){
			selectAllChk.attr("checked" ,'true');
		}else{
	...
	function revSelect(obj){
	    ...
	    for(var i=0; i<records.length;i++){
	    	...
	    	if(obj.checked){
		    	...
		    	//添加结果区
				var jsonObj = createObj(checkObj) ;
