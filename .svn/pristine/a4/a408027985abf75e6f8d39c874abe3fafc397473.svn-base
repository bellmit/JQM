	...
	<script type="text/javascript">
    	...
    	//tab页权限处理
		 nui.ajax({
            ...
            type:'POST',
            data:{funcCode:"ATS_JSGL_SQSSUM,ATS_JSGL_DEBT_DATA_QUERY,ATS_JSGL_MANAGER_QUERY,ATS_JSGL_PROPDATA"},
            cache:false,
            ...
            success:function(text){
	        	...
	        	if(result){	        	
	        		...
	        		for(var k=0;k<result.length;k++){
	        			if(result[k].RES_ID=="ATS_JSGL_MANAGER_QUERY"){
							tempArray[0]=result[k];
						}
	        			if(result[k].RES_ID=="ATS_JSGL_SQSSUM"){
							tempArray[1]=result[k];
						}
	        			if(result[k].RES_ID=="ATS_JSGL_DEBT_DATA_QUERY"){
							tempArray[2]=result[k];
						}
	        			if(result[k].RES_ID=="ATS_JSGL_PROPDATA"){
