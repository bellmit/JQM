 ...
 <div style="width:100%;height:100%; "  borderStyle="border:solid 1px #aaa;">
	<div style="height:10px;">
		...
		onuploaderror="onUploadError" onfileselect="onFileSelect" />
		 ...
		 <a class='nui-button' plain='false' iconCls="icon-upload" plain="false" onclick="startCustomExecl()">上传</a>
		 <input id="customName" class="nui-hidden" name="customName"/>
	</div>
 </div>	
	<!-- 方法使用：
	...
	<script type="text/javascript">
    	...
    	*/
	    ...
	    var excelData={
			...
			var data2 = {};
	    function startCustomExecl(){
	        
	        var cStatus="1";
	        var lUnderwriterTemp="1";
	        var json =nui.encode({userName:window.parent.userId,cStatus:cStatus,lUnderwriterTemp:lUnderwriterTemp});
				nui.ajax({
				    ...
				    success: function (text) {
				        var data=text.underwriterInport[0];
				        if(data!=null){
				           for ( var i in data ){
				                 if(data[i]!=""){
				                    var toNum=data[i];
				           ...
				           customName=JSON.stringify(data2);
	                       startUpload();
				        }else{
				           window.parent.nui.alert("上传失败，请联系交易员配置导入模板.");
				        }
				    }
				});
