	...
	<script type="text/javascript">
    	...
    	//tab页权限处理
		 nui.ajax({
            ...
            type:'POST',
            data:{funcCode:"ATS_TCGL_TCFXYSB,ATS_TCGL_T0TCCE,ATS_TCGL_T1TCCE"},
            cache:false,
            ...
            success:function(text){
	        	...
	        	if(result){
	        		...
	        		for(var k=0;k<result.length;k++){
	        			if(result[k].RES_ID=="ATS_TCGL_TCFXYSB"){
							tempArray[0]=result[k];
						}
						if(result[k].RES_ID=="ATS_TCGL_T0TCCE"){
							tempArray[1]=result[k];
						}
						if(result[k].RES_ID=="ATS_TCGL_T1TCCE"){
