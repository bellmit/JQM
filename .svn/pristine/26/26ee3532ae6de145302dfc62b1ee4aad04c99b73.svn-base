		                ...
		                <table id="table1" class="table" style="height:100%;table-layout:fixed;">
		                	<tr>
		                        ...
		                        <td style="width:28%" align="left">
		                            <input id="processDate_qtzl_begin" name="criteria/_expr[2]/processDate" class="nui-datepicker" width="100px"/>
		                            <input class="nui-hidden" type="hidden" name="criteria/_expr[2]/_op" value=">=">
							 		- 
							 		<input id="processDate_qtzl_end" name="criteria/_expr[5]/processDate" class="nui-datepicker" width="100px"/>
...
<!-- </div> -->
	...
	<script type="text/javascript">
	    ...
	    //初始化查询条件业务日期值为当天
	    var date = new Date();
		nui.get("processDate_qtzl_begin").setValue(date);
		nui.get("processDate_qtzl_end").setValue(date);
	    ...
	    grid_QTZL.on("rowdblclick", function (e) {
	    	rowdblclickFun("JY_QTZL/CFJYProductProcessForm_detailOperator_show_QTZL.jsp", "其他指令/建议明细展示", 780, 630, e, grid_QTZL);
	    });
	    //新增
	    function add_QTZL() {
	    	add("JY_QTZL/CFJYProductProcessForm_QTZL.jsp", "其他指令/建议新增记录", 780, 550, grid_QTZL);
	    }
	    //编辑
	    function edit_QTZL() {
	    	edit("JY_QTZL/CFJYProductProcessForm_QTZL.jsp", "其他指令/建议编辑数据", 780, 550, grid_QTZL);
	    }
	    //废弃
	    ...
	    function edit(jspUrl, title, width, height, grid, editData) {
	        ...
	        if (row) {
	        	...
	        	}else if(processStatus=="0" || processStatus=="1"||processStatus=="7"){
		...
		function confirm(grid_confirm){
		    ...
		    if(rows.length > 0){
	    		...
	    		for(var i=0; i<rows.length; i++){
		        	...
		        	}else if(processStatus=="7"){//投资经理下单操作权限认证   
		        		var processId = record.processId;
		        		nui.ajax({
		        			url:"com.cjhxfund.jy.ProductProcess.CFJYProductProcessBiz.getProductProcessHandleByProcessId.biz.ext",
			                type:'POST',
	    ...
	    //全屏显示
		function fullScreen(){
			window.open('<%=request.getContextPath()%>/ProductProcess/JY_QTZL/CFJYProductProcessList_QTZL.jsp','newwindow','height=100,width=400,top=0,left=0,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
		}
