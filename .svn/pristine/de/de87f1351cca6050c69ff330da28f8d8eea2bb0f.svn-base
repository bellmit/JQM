...
<%
  ...
  System.out.println("获取URL类型----------------------"+serverType);
  String mServerUrl = "";
  String mHttpUrl = "";
  ...
  if (DbaObj.OpenConnection()){
    ...
    try{
      ...
      if (result.next()){
        ...
        System.out.println("文档路径：============================="+FileAdd);
...
function Load(){
  try{
    ...
    webform.WebOffice.WebUrl="<%=mServerUrl%>?Filepath=<%=URLEncoder.encode(FileAdd,"utf-8") %>&FileType=2";      //WebUrl:系统服务器路径，与服务器文件交互操作，如保存、打开文档，重要文件
