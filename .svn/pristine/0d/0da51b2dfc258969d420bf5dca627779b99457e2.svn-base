...
nui.extend(nui.bps.ProcessGraph, nui.Control, {
	...
	_draw:function (data, context, imgPath) {
		...
		var imgTemplate = '<IMG id="{id}"' +
						  ...
						  ' onclick="showSubProcess(\'{clickId}\',\'{processId}\',\'{activityId}\')"' +
						  ' />';
...
function showSubProcess(id,processId,activityId){
	var element = document.getElementById(id);
	if (element && processId!="undefined" && activityId!="undefined"){
		//window.open("http://localhost:8080/default/com.cjhxfund.fpm.onetomanyRaise.ReaderOtmRaiseFlow.flow?processInstID=12865");
		var json={activityInstId:activityId};
		var urlStr = "com.cjhxfund.fpm.onetomanyRaise.common.querySubProcess.biz.ext";
		nui.ajax({
			type : "post",
			url : urlStr,
			data : json,
			...
			success : function(text) {
				var subProcessId = text.subProcessId[0];
				if(subProcessId!="0" && subProcessId!="-1"){
					var requestUrl = contextPath+"/onetomanyRaise/showSubProcess.jsp?processInstID=" + subProcessId;
...
function drawStates (states,participants,context,imgTemplate,participantsListTemplate) {
	...
	for (prop in states) {
		...
		tmpTemplate = tmpTemplate.replace('{clickId}',stat.id);
		tmpTemplate = tmpTemplate.replace('{processId}',stat.extendAttribute.processInstId);
		tmpTemplate = tmpTemplate.replace('{activityId}',stat.extendAttribute.activityInstId);
		...
		content += "<tr><td>" + context.partiName + "</td><td>" + 
					context.startTime + "</td><td>" + context.endTime + "</td></tr>";
			//<td>" + context.timeOut + "</td>
     	if(context.showParticipants == "true" && participants){
     		...
     		var N = context.N;
     		if(statId.indexOf("$")>-1){
     			if(stat.currentState == "2_0"){
     				var realStartId = statId.split("$")[0];
     				if(realStartId != "manualActivity1"){
     					isShow = "true";
     				}
     			}
     		}
     		if(partis && partis.length > 0 && isShow != "true"){
				for (var i = partis.length - 1; i >= 0; i--) {
					partis[i].startTime = partis[i].startTime || "-";
					partis[i].endTime = partis[i].endTime || "-";
					content += "<tr><td style='padding-left:5px;padding-right:5px'>" + partis[i].participant + "</td><td style='padding-left:5px;padding-right:5px'>" + partis[i].startTime + "</td><td style='padding-left:5px;padding-right:5px'>" + partis[i].endTime +
					"</td></tr>";
					//<td style='padding-left:5px;padding-right:5px'>" + (partis[i].isTimeOut=="Y"?Y:N) + "</td>
				};
				content += "</table>";
				...
				if(statId.indexOf("$")>-1){
					var realStartId = statId.split("$")[0];
					if(realStartId == "manualActivity1"){
						createParticipant = partis;
					}
				}
			}
     		else{
				if("manual"==stat.type){
					var list = queryActiPartiPerson(stat.extendAttribute,context.procInstID);
					if(list!=null&&list.length>0){
						for (var i = list.length - 1; i >= 0; i--) {
							list[i].startTime = "-";
							list[i].endTime = "-";
							content += "<tr><td style='padding-left:5px;padding-right:5px'>" + list[i].name + "</td><td style='padding-left:5px;padding-right:5px'>" + list[i].startTime + "</td><td style='padding-left:5px;padding-right:5px'>" + list[i].endTime +
							"</td></tr>";
						};
						content += "</table>";
						//增加活动状态的table
						...
						var isTrue = queryIsCreateParticipant(stat.extendAttribute,context.procInstID);
						if(isTrue == "true" && createParticipant != null && createParticipant != ""){							
								for (var i = createParticipant.length - 1; i >= 0; i--) {
									createParticipant[i].startTime = "-";
									createParticipant[i].endTime = "-";
									content += "<tr><td style='padding-left:5px;padding-right:5px'>" + createParticipant[i].participant + "</td><td style='padding-left:5px;padding-right:5px'>" + createParticipant[i].startTime + "</td><td style='padding-left:5px;padding-right:5px'>" + createParticipant[i].endTime +
									"</td></tr>";
								};
								content += "</table>";
							...
							//增加活动状态的table
							content = "<table style='white-space:nowrap;text-align:center;border-collapse:collapse;' cellspacing='0px' border='1px'>";
							content += "<tr><td style='padding-left:5px;padding-right:5px'>流程还未指定参与者!</td></tr>";
							content += "</table>";
						}						
					}
				}else{
...
function drawLines (states,transactions,context) {
	...
	for (var i = 0, l = transactions.length; i < l; i++) {
		...
		if (transaction.name) {
			var exLength = 20*context.zoomQuotiety;
    		var width = transaction.name.length  * context.zoomQuotiety * 2 * 10;
    		var left = mid.x - width / 2 + 10*context.zoomQuotiety;
    		var top = mid.y - context.fontSize + 10*context.zoomQuotiety;
    		var height = context.fontSize  * context.zoomQuotiety * 3 ;
    		if(!transaction.extendAttribute['1h1']){
    			left += exLength;
    			top += exLength;
    		}
    		var tmp = drawText(transaction.name, context.fontSize, left, top, width, height, context.zoomQuotiety);
    		if(transaction.name=="是"||transaction.name=="否") tmp = "";
    		result += tmp;
    	};
 ...
 * 陈文豪-2016-11-16
 */
function queryActiPartiPerson(extendAttribute,processInstID){
	var returnData = ""; 
	var activityDefID = extendAttribute.activityDefID;
	var processDefId =extendAttribute.processDefId;
	var json = nui.encode({activityDefID:activityDefID,
							processDefId:processDefId,
							processInstID:processInstID});
	nui.ajax({
		type:"post",
		url:"com.cjhxfund.fpm.bpsExpend.process.getActiPartiPerson.biz.ext",
		data:json,
		cache:false,
		async:false,
		contentType:'text/json',
 ...
 * 姜坎骞-2017-8-1
 */
function queryIsCreateParticipant(extendAttribute,processInstID){
	var returnData = ""; 
	var activityDefID = extendAttribute.activityDefID;
	var processDefId =extendAttribute.processDefId;
	var json = nui.encode({activityDefID:activityDefID,
	...
	nui.ajax({
		type:"post",
		url:"com.cjhxfund.fpm.bpsExpend.process.isCreateParticipant.biz.ext",
		data:json,
		cache:false,
		async:false,
		contentType:'text/json',
        success: function (data) {
        	returnData = data.isTrue;
        }	
    });
	return returnData;
}