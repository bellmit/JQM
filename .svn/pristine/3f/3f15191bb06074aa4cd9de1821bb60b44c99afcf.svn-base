...
function fileQueueError(file, errorCode, message) {
	try {
		...
		if (errorName !== "") {
			window.parent.nui.alert(errorName);
			return;
		}
		...
		case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
			...
			break;
		default:
			window.parent.nui.alert(message);
			break;
		}
...
function fileQueued(file){
	...
	if (isDigit(file.name)==false){
	    ...
	    window.parent.nui.alert("文件名不合法！只能由 中文、数字、字母、下划线、横线、括号组成！");
		return; 
	}
}
...
function addReadyFileInfo(fileid,fileName,message,status,size1){
	...
	col5.style.width="80";
	//新增上传文件
	var newRow = { lAttachId:fileid,sysid:fileid,vcAttachName:fileName,name:fileName,message:message,status:status,vcAttachSize:attachSize,attachSize:attachSize, vcUserId:userId, vcUserName:userName, dCreateTime:new Date()};
    grid.addRow(newRow, 0);
    setGridHeight();
    if(window.parent.refreshFile){
