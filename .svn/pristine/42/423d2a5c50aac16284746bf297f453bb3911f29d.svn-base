	...
	<script type="text/javascript">
    	...
    	//tab页权限处理
		 nui.ajax({
            ...
            type:'POST',
            data:{funcCode:"ATS_ZHXXCX_CPJGGL,ATS_ZHXXCX_CPTZFX,ATS_ZHXXCX_MOMBB,ATS_ZHXXCX_JXFXBGXZ"},
            cache:false,
            ...
            success:function(text){
	        	...
	        	if(result){
	        		...
	        		for(var k=0;k<result.length;k++){
	        			if(result[k].RES_ID=="ATS_ZHXXCX_CPJGGL"){
							tempArray[0]=result[k];
						}
						if(result[k].RES_ID=="ATS_ZHXXCX_CPTZFX"){
							tempArray[1]=result[k];
						}
						if(result[k].RES_ID=="ATS_ZHXXCX_MOMBB"){
							tempArray[2]=result[k];
						}
						if(result[k].RES_ID=="ATS_ZHXXCX_JXFXBGXZ"){
