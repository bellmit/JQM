...
public class ExcelTemplate {
	 ...
	 */
	public ExcelTemplate(String templateFile,String outputFile,boolean flag){
	...
	private void initialize(HSSFSheet sheet){
		if(isCover){
	        ...
	        this.fieldNames=new String[firstCells];
	        for(short c = 0; c < firstCells; c++){
	        	...
	        	if(cell!=null)
	        	{
	            	...
	            	{
	            		value=value.trim();
	            		this.fieldNames[c]=value;//初始化内容字段
	            	}
	        	}
	        }
	        this.startRow =rows;
		}else{
			initializeNotCover(sheet);
		}

	}
	private void initializeNotCover(HSSFSheet sheet){
                		...
                		if(cell.getCellType()==HSSFCell.CELL_TYPE_STRING){
                        	...
                        	if(value!=null){
                        		...
                        		if(value.startsWith(TITLE_FLAG)){
                        			...
                        			cell.setCellValue((RichTextString)new HSSFRichTextString(content));
	...
	private void generateContentDatas(List<DataObject> resultset,HSSFWorkbook wb,HSSFSheet sheet){
        ...
        for(Iterator it=resultset.iterator();it.hasNext();autoRowId++){
        	...
        	int number = sourceRow.getPhysicalNumberOfCells();//获取表格所有列
        	for(int i=0;i<number;i++){
        		...
        		if(fieldNames[i]!=null){
    				...
    				if(sourceCell!=null&&
	    				...
	    				HSSFCell cell=row.createCell((short)i);
	    				cell.setCellType(HSSFCell.CELL_TYPE_STRING);
	    				cell.setCellValue((RichTextString)sourceCell.getRichStringCellValue());
    				}
    			}
        	}
            ...
            for (short i = 0; i <= oldrow.getPhysicalNumberOfCells(); i++) {
                ...
                if (oldcell != null) {                    
                    ...
                    case HSSFCell.CELL_TYPE_STRING:
                    	cell.setCellType(HSSFCell.CELL_TYPE_STRING);
                    	cell.setCellValue((RichTextString)oldcell.getRichStringCellValue());
                    	break;
