	...
	<script type="text/javascript">
    	...
    	var queryUserType = "<%=request.getParameter("queryUserType")%>";
		 nui.ajax({
            ...
            type:'POST',
            data:{funcCode:"ATS_JY_TZZCJFZXX,ATS_JY_ZGJHDCZCXX,ATS_JY_CCXX,ATS_JY_XXSJ"},
            cache:false,
            ...
            success:function(text){
	        	...
	        	if(result){
	        		...
	        		for(var k=0;k<result.length;k++){
	        			if(result[k].RES_ID=="ATS_JY_TZZCJFZXX"){
							tempArray[0]=result[k];
						}
						if(result[k].RES_ID=="ATS_JY_ZGJHDCZCXX"){
							tempArray[1]=result[k];
						}
						if(result[k].RES_ID=="ATS_JY_CCXX"){
							tempArray[2]=result[k];
						}
						/* if(result[k].RES_ID=="ATS_JY_XXSJ"){
