...
<fieldset>
		...
		<div id="oldInstructInfo" style="padding-top: 5px; height: auto; width: 100%;">
			...
			<div class="infoLabel2">
				<label> 回购利率(%) </label><input id="enRepoRate"
					class="nui-textbox" required="true" vtype="float" onblur="calEmFaceAmount()" enabled="false" />
			</div>
	...
	<fieldset>
		...
		<div id="DirectionInfo"
			...
			<div class="infoLabel2">
				<label> 质押券代码 </label>
					<div name="vcReportCode" class="mini-autocomplete" id="vcReportCode"
			...
			<div class="infoLabel2">
				<label> 质押数量 </label><input id="vcMortagageQty" class="nui-textbox" onvaluechanged="calcEnFaceAmount2" />
			</div>
			<div class="infoLabel2" style="width: 100%;">
...
//         var bond = instructParameter.bond;
        ...
        nui.get("vcReportCode").url = "com.cjhxfund.ats.sm.comm.TBondBaseInfoManager.GetSSFundBond.biz.ext?fundId="+data.vcProductCode;
    }
    	...
    	nui.ajax({
            ...
            success: function (e) {
                if (e.exception == null) {
                    ...
                    nui.get("vcReportCode").setValue(data["vcStockName"]);
                    nui.get("vcReportCode").setText(data["vcReportCode"]);
			        ...
			        nui.get("vcMortagageQty").setValue(data["vcMortagageQty"]);
                } else {
                    nui.alert("系统异常", "系统提示");
                }
            }
    ...
    function calEmFaceAmount() {
        ...
        var temp = interest.indexOf('.');
        if (temp > 0) {
            interest = interest.slice(0, temp + 3);
        }
		...
		function calcEnFaceAmount2(e){
			...
			var faceAmountValue = enFaceAmount2.getValue(); //券面金额
			var enPledgeRatio = nui.get("enMortagageRatio").getValue();
			...
			nui.get("enMortagageRatio").setValue(enRatio);
		}
    //组装完整指令/建议参数
    var directionParam = {};
	...
	function getStockName(){
			nui.get("vcStockName").setValue(nui.get("vcReportCode").getValue());
			var enFaceAmount2 = nui.get("enFaceAmount2");
			var lMortagageAmount = nui.get("lMortagageAmount");			
			var enRatio = nui.get("enMortagageRatio");
			var faceAmount = enFaceAmount2.getValue();
