              ...
              if (text.busDates.length>0) {
              	 ...
              	 for(var j=0; j<text.busDates.length; j++){
					...
					busDateArr.add(busDate+"_"+vc_cpdm);
						...
						//设置tab属性
						var tab = {title:productCode+productName+busDate, url:"<%=request.getContextPath() %>/ProductProcess/zhxxcx/CFJY_zhxxcx_hsfagzb_detail.jsp?vCFundCode="+productCode+"&busiDateBegin="+busDate+"&busiDateEnd="+busDate+"&queryUserType="+queryUserType+"&vCFundName="+vCFundName, showCloseButton: true };
						//设置tab
						tabs.addTab(tab);
						//当i=0时模拟点击事件
			 			if(j == 0){
