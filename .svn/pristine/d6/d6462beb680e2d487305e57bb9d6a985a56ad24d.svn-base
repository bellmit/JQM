			...
			<table class="instructTable" border="0" style="width: 100%;">
				<tr>
					...
					<td style="width: 35%;" align="left">
						...
						<input id="autoCalc" name="autoCalc"  class="nui-checkbox" oncheckedchanged="autoCalcPrice" checked="true" text="联动"/>
					</td>
	...
	<script type="text/javascript">
        		...
        		success:function(resp){
					...
					if(returnJson.exception == null){//客户下达指令/建议时间>产品参数配置时间
						 if(returnJson.rtnCode == "<%=com.cjhxfund.commonUtil.Constants.ATS_ORDER_OVER_DEAL_TIME%>"){ 
							window.parent.parentConfirm("该投资指令/建议下达时间过晚，无法完全保证交易达成，是否继续？", "提示",
					            function (action) {
					                if (action == "ok") {
		...
		//指令/建议下达或修改start
 		function instructionAvailIssue(instructJson,doClose){
	            ...
	            success: function (text) {
	            	...
	            	if(returnJson.exception == null){
	            		if(returnJson.rtnCode == "<%=com.cjhxfund.commonUtil.Constants.ATS_SUCCESS%>"){
	            			...
	            			window.parent.parentAlert("投资指令/建议下达成功！"+returnJson.rtnMsg,"提示",function(){
                            	if(doClose){
                            		window.parent.closewindow("ok");
                            	}
                            });
   	 	...
   	 	//正式修改指令/建议信息
   	 	function formalInstructionEntry(instructJson,doClose, riskFlag){
   	 		var tooltip = nui.loading("正在处理中,请稍等...","提示");
   	 		nui.ajax({
        ...
        function riskControlTrial(e){
        	...
        	var parameter = capsulationInstructParameter();
        	parameter["callRiskType"] = "1";//风控校验类型：1-风控试算，2-投资/建议下达
        	var json = null;
        	json = {instructParameter:parameter};
        	 //风控试算start
		...
		function SetData(data) {
			...
			if (data["operatorType"] == "2") { //修改指令/建议
			    operatorType="2";
