...
<%
	...
	String iDataUrl = host+"/iData/vision/redirect.jsp?fillDate="+fillDate+"&userid="+userid+"&seriesAccount="+seriesAccount;
 %>	
...
<body>
	...
	<div title="center" region="center" style="border:0;padding-left:5px;padding-top:0px;width:99%;height:99%;">
       ...
       <div id="noldingsDetailTabs" onactivechanged="activeChangeTabs" class="nui-tabs  bg-toolbar" activeIndex="0" style="width:100%;height:100%;">
		    <div name="combination_distribution_tab" id="combination_distribution_tab" title="组合债券明细" url="<%=iDataUrl %>" visible="true" >
		    </div>
	...
	<script type="text/javascript">
		...
		combination_distribution_tab.url = idataUrl+"&resid=I8a49602f0af779fb01589aab33c6435a";
		
		function activeChangeTabs(){
			var activeTabTitle = nui.get("noldingsDetailTabs").getActiveTab().title;
			//组合债券分布
			if(activeTabTitle.indexOf("组合债券明细")!=-1){
				var combination_distribution_tab = nui.get("noldingsDetailTabs").getTab("combination_distribution_tab");
				combination_distribution_tab.url = idataUrl+"&resid=I8a49602f0af779fb01589aab33c6435a";
			}
			//组合期限分布
			if(activeTabTitle.indexOf("组合期限分布")!=-1){
				...
				combination_limit_tab.url = idataUrl+"&resid=I8a49602f0af779fb01589ab64f2d442d";
			}
			//组合信用分布
			if(activeTabTitle.indexOf("组合信用分布")!=-1){
				var combination_credit_tab = nui.get("noldingsDetailTabs").getTab("combination_credit_tab");
				combination_credit_tab.url = idataUrl+"&resid=I8a49602f0af779fb01589ac8888b44d3";
			}
			//信用债发行人行业分布
			if(activeTabTitle.indexOf("信用债发行人行业分布")!=-1){
				...
				combination_issuer_tab.url = idataUrl+"&resid=I8a49602f0af779fb01589acdeab2451e";
			}
    	}
    </script>
</body>
