...
<div id="vunitstockGrid" dataField="vunitstockList"
    ...
    <div property="columns">
        ...
        <div field="vcAvailablequantityT1" width="100" decimalPlaces="0" headerAlign="center" allowSort="true" dataType="int" numberFormat="n0">T+1可质押数量</div>
	...
	var copySelectData = function (rows) {
	    ...
	    for (var i = 0; i < temp.length; i++) {
	        ...
	        if (object["vcAvailablequantityT0"]) {
	            object["enFaceAmount"] = object["vcAvailablequantityT0"] / 100;//T+0可质押数量除以100
	        } else {
	            object["enFaceAmount"] = 0;
	        }
	        //默认质押比率
	        if (object["enCbValueNetValue"]) {// 净价（T-1）
	            var enCbValueNetValue = object["enCbValueNetValue"];
	            var index = enCbValueNetValue.indexOf(".");
	            if(index>0){
	            	enCbValueNetValue = enCbValueNetValue.slice(0, index);//净价（T-1）>100?100:净价（T-1）
	            	object["enMortagageRatio"] = enCbValueNetValue > 100 ? "100" : enCbValueNetValue.toString();
	            }
	        } else {
	            object["enMortagageRatio"] = 0;
	        }
	        //计算可做回 购金额          （净价（T-1）>100?100:净价（T-1））*（T+0可质押数量除以100）
