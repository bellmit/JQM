	conditionForm.on("load",function(e){
	 							...
	 							for(var j=0; j<dealPosition.length; j++){
	 								for(var i=0; i<availableData.length; i++){
	 									if(availableData[i].VC_ACCOUNT == dealPosition[j].vcCombiNo && availableData[i].VC_SYMBOL == dealPosition[j].vcStockCode){
	 										var vcAvailablequantityTmpT0 = parseInt(availableData[i].VC_AVAILABLEQUANTITY_INQUIRY_T0.toString());
	 										var vcAvailablequantityTmpT1 = parseInt(availableData[i].VC_AVAILABLEQUANTITY_INQUIRY_T1.toString());
        ...
        function search(){
     		...
     		commandInformation["relateType"] = "03";
         ...
         function reset(){
        	 ...
        	 if(vcFundCode.enabled){
        		 vcFundCode.setValue("");
        		 vcFundCode.setText("");
        	 }
        	 if(vcCombiNo.enabled){
        		 vcCombiNo.setValue("");
        		 vcCombiNo.setText("");
        	 }
        	 nui.get("vcBondAppraise").setValue("");
        	 nui.get("minLimiteLeft").setValue("");
        	 nui.get("maxLimiteLeft").setValue("");
             //tabDealRole.reset();
          }
          //整数校验
 		...
 		//联动方法
 		var selectFund = function () {
 		    var id = nui.get("vcFundCode").getValue();
 		    var vcCombiCombo = nui.get("vcCombiNo");
 		    $.ajax({
