...
-->
<head>
<title>投资指令/建议管理-修改报价（中标确认环节特有）</title>
...
<body style="width: 100%;height: 100%;overflow: hidden;">
	    ...
	    <table style="width:100%; table-layout:fixed;" border="0" class="nui-form-table">
			...
			<tr>
			  ...
			  <td colspan="1" width="35%">
			    <input  class="nui-dictcombobox" id="stockissueinfo.vcStockType" name="stockissueinfo.vcStockType"  allowInput="true"
	...
	<script type="text/javascript">
			...
			nui.ajax({
			    ...
			    success: function (text) {
					...
					if(returnJson.workItems.length>0){
						...
						//applyInst
						applyInst=returnJson.applyInst;//传入的指令/建议信息
		...
		function onOk(){
			...
			bizId:bizId,workitme:workitme,applyInst:applyInst,isNoticeAdviser:1}); 
			//console.log(json);
			form.loading();//加载遮罩
			    ...
			    success: function (text) {
			    	...
			    	*/
			    	if(text.ret==3){
			    		...
			    		nui.alert("修改成功，系统已经自动提交修改数据，请确认！！","系统提示",function (a){
				    		//关闭界面
				    		CloseWindow("cancel");
			    		});
			    	}else if(text.ret==-4){
