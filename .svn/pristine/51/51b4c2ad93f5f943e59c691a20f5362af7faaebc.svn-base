...
//行双击时弹出页面展示该指令/建议详细信息--正逆回购
function rowdblclickFun_common_ZNHG(jspUrl, title, width, height, e, grid_confirm, pageType, roleType) {
    ...
    if (row) {
    	nui.open({
            ...
            onload: function () {
                ...
                var data = {pageType:pageType,roleType:roleType,record:{zhfwptjyznhg:row}};
                //直接从页面获取，不用去后台获取
...
//确认--所有业务通用
function confirm_common_ZNHG(grid_confirm){
...
//编辑--所有业务通用
function edit_common_ZNHG(jspUrl, title, width, height, grid, pageType, roleType) {
...
//废弃--所有业务通用
function remove_common_ZNHG(grid_remove){
								  	...
								  	$.ajax({
								          ...
								          success:function(text){
								              ...
								              if(returnValidStatus=="0" || returnValidStatus=="3"){
							                      ...
							                      var json = nui.encode({zhfwptjyznhgs:[{processId:rows[0].processId, processStatus:processStatus, abandonedReasons:value}]});
...
function rowdblclickFun_common_confirm_ZNHG(jspUrl, title, width, height, e, grid_confirm, editJspUrl) {
    var row = e.record;//行对象值
    if (row) {
		...
		if(processStatus=="-2" && (validStatus=="0"||validStatus=="3") && hasPermission==true){
			nui.open({
                ...
                onload: function () {
                    var iframe = this.getIFrameEl();
                    var data = {pageType:"edit",roleType:"JYY",record:{zhfwptjyznhg:row}};
                    //直接从页面获取，不用去后台获取
                    iframe.contentWindow.setFormData(data);
                },
                ondestroy: function (action) {
		...
		//打开确认界面
			...
			nui.open({
                ...
                onload: function () {
                    var iframe = this.getIFrameEl();
                    var data = {pageType:"confirm",roleType:"JYY",record:{zhfwptjyznhg:row}};
                    //直接从页面获取，不用去后台获取
                    iframe.contentWindow.setFormData(data);
                },
                ondestroy: function (action) {//弹出页面关闭前
...
//退回--所有业务通用
function goBack_common_confirm_ZNHG(grid_goBack){
...
//确认--所有业务通用
function confirm_common_confirm_ZNHG(grid_confirm){
    var rows = grid_confirm.getSelecteds();
    if(rows.length > 0){
...
//编辑--所有业务通用
function edit_common_confirm_ZNHG(jspUrl, title, width, height, grid) {
    var row = grid.getSelected();
    if (row) {
    	//交易状态：0-有效；1-无效-修改；2-无效-废弃；3-有效-退回；4-无效-退回；
    	var validStatus = row.validStatus;
