                ...
                <table id="table1" class="table" style="height:100%;table-layout:fixed;">
                	<tr>
                        ...
                        <td colspan="1" width="15%" align="left">
                            <input id="vCFundCode_T0YHJ" class="nui-buttonedit" name="paramObject/vCFundCode" onbuttonclick="ButtonClickGetFundName_T0YHJ"/>
                        	<%-- 设置默认的债券基本信息表当前业务日期 --%>
                        	<input id="preDate_T0YHJ" class="nui-hidden" name="paramObject/preDate" />
                        </td>
                        <td class="form_label" width="60px" align="right">
							债券代码:
                        </td>
                        <td colspan="1" width="15%" align="left">
                            <input id="vCReportCode_T0YHJ" class="nui-textbox" name="paramObject/vCReportCode"/>
                        </td>
                        <td class="form_label" width="60px" align="right">
							债券名称:
                        </td>
                        <td colspan="1" width="15%" align="left">
                            <input id="vCStockName_T0YHJ" class="nui-textbox" name="paramObject/vCStockName"/>
                        </td>
                        <td align="right" colspan="10">
                            <input class='nui-button' plain='false' text="查询" iconCls="icon-search" onclick="search_T0YHJ(true)"/>
                            <input class='nui-button' plain='false' text="重置" iconCls="icon-cancel"  onclick="resetDire_T0YHJ()"/>
							<input class='nui-button' plain='false' id="export_T0YHJ" text="导出" iconCls="icon-download" onclick="export_T0YHJ()"/>
                        </td>
       ...
       <div class="nui-fit">
            <div 
            	  ...
            	  债券名称
		                ...
		                <div field="TRUSTEESHIP" name="TRUSTEESHIP" headerAlign="center" allowSort="true" align="left" width="80px">
            	  托管场所
		                </div>
            	  ...
            	  主体评级      
		                ...
		                <div field="L_SALE_AMOUNT_COUNT" name="L_SALE_AMOUNT_COUNT" headerAlign="center" allowSort="true" align="right" width="240px">
	  			  可用总数量       
                        </div>
...
<script type="text/javascript">
	...
	//设置默认时间
	var preDate = "<%=preDate%>";
	nui.get("preDate_T0YHJ").setValue(preDate);
	
	var grid_T0YHJ = nui.get("datagrid_T0YHJ");//T0银行间可用质押券明细表
    ...
    self.setInterval("autoRefreshFun()",60000*3);//设置自动刷新时间默认3分钟
	
	<%-- 当日T+0头寸差额开始... --%>
	//查询
	function search_T0YHJ(flag){
		havingAlert = flag;
		nui.get("exclusivelyShowSection").setValue("1");
		search(grid_T0YHJ, "#form_T0YHJ");
	}
	//获取查询条件的基金名称
	function ButtonClickGetFundName_T0YHJ(e){
        ButtonClickGetFundName(this, null);
	}
    //行双击时弹出页面展示该指令详细信息
	grid_T0YHJ.on("rowdblclick", function (e) {
		rowdblclickFun("CFJY_zhxxcx_T0YHJ_detail.jsp", "当日T+0头寸差额-银行间可用质押券明细", 710, 400, e, grid_T0YHJ);
	});
	//合并单元格
	grid_T0YHJ.on("load", function () {
		var length = grid_T0YHJ.data.length;
		var datas = grid_T0YHJ.data;
    	if(length == 0 ){
    		if(havingAlert){
    			nui.alert("没有符合筛选的数据！");
    		}
    		...
    		for(var i=0;i<length;i++){
				...
				for(var k=0;k<length;k++){
					if(datas[k].L_FUND_ID == datas[i].L_FUND_ID){
						...
						if("上清所" == datas[k].TRUSTEESHIP){
							sqcount +=parseFloat(datas[k].L_SALE_AMOUNT);
						}else if("中债登" == datas[k].TRUSTEESHIP){
							zzcount +=parseFloat(datas[k].L_SALE_AMOUNT);
						}
					}
				}
				datas[i].L_SALE_AMOUNT_COUNT = "上清:"+formatNumber(sqcount,2,1)+" ， 中债:"+formatNumber(zzcount,2,1);
			}
	    	grid_T0YHJ.mergeColumns(["L_FUND_ID","VC_FUND_CODE","VC_FUND_NAME","L_SALE_AMOUNT_COUNT"]);
    	}
    	havingAlert = false;
    });
    /** 
    ...
    //重置指令信息
	function resetDire_T0YHJ(){
		var form = new nui.Form("form_T0YHJ");
		form.reset();
	}
	//导出
	function export_T0YHJ(){
	    exportSubmit("export_T0YHJ", "form_T0YHJ", "当日T+0头寸差额-银行间可用质押券明细","TCXXCX");
	}
   //精确加
   function add(a, b) {
	    var c, d, e;
