...
<body style="margin: 0px;padding: 0px;overflow: hidden;width: 100%;height: 100%;" onload="onloadFun()">
	...
	<script type="text/javascript">
	    ...
	    grid_QTZL.on("rowdblclick", function (e) {
	    	rowdblclickFun("JY_QTZL/CFJYProductProcessForm_detailOperator_show_QTZL.jsp", "其他指令/建议明细展示", 800, 630, e, grid_QTZL, "JY_QTZL/CFJYProductProcessForm_QTZL.jsp");
	    });
	    //确认
	    ...
	    //全屏显示
		function fullScreen(){
			window.open('<%=request.getContextPath()%>/ProductProcess/JY_QTZL/CFJYProductProcessList__queryConfirm_QTZL.jsp','newwindow','height=100,width=400,top=0,left=0,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
		}
