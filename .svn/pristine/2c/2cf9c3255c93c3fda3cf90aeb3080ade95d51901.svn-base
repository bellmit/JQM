...
public class JavaExportDoc {
	...
	private void getData(Map<String, Object> dataMap, DataObject obj) {
		...
		//判断赎回行权年是否为空
		if(redemptionYear != 0){
			resale =  "赎回：" + redemptionYear + "+" + (obj.getInt("enExistLimite") - redemptionYear);
		}
				
		//判断回售行权年是否为空
		if (resaleYear != 0) {
			resale = resale +"," + "回售：" + resaleYear + "+" + (obj.getInt("enExistLimite") - resaleYear);  
		} 
		...
		//获取主体评级
		String subjectrating = getDictMap("creditRating").get(obj.getString("cIssueAppraise"));
		//获取债券评级
		...
		//发行人名称
		dataMap.put("IssuerName", obj.getString("vcIssuerName") == null ? "" : obj.getString("vcIssuerName"));
		//债券代码
		...
		//主体评级
		dataMap.put("subjectrating", subjectrating == null ? "asa" : subjectrating);
		...
		//付息方式
		String frequency = getDictMap("CF_JY_FREQUENCY").get(obj.getString("vcModeRepayment"));  
		...
		//首次付息日
		dataMap.put("InterestDate", obj.getString("lBegincalDate") == null ? "" : DateUtil.formatDate(obj.getString("lBegincalDate"),"yyyy-MM-dd"));
		
		//债券年限
		dataMap.put("categorydate", obj.getString("enExistLimite") == null ? "" : obj.getString("enExistLimite"));
		
		//行权(赎回,回售)
		dataMap.put("exercise", "asda");
