...
-->
 <%
 	long key = JDBCUtil.getNextSequence("SEQISSUERNO");
 	long code = JDBCUtil.getNextSequence("seqissuercode");
	            ...
	            <table style="width:100%;height:100%;table-layout:fixed;" class="nui-form-table">
	                ...
	                <tr>
	                    ...
	                    <td colspan="1">
	                    	...
	                    	<input class="nui-hidden" id="lIssuerId" style="width: 95%;"  name="issuerInfo.lIssuerId" value="<%=key %>"/>
	                    	<input class="nui-hidden" id="lIssuerIdO32" style="width: 95%;"  name="issuerInfo.lIssuerIdO32" value="0"/>
	                    	<input class="nui-hidden" id="vcIssuerCode" style="width: 95%;"  name="issuerInfo.vcIssuerCode" value="<%=issuerCode %>"/>
	                    </td>
	                </tr>
	                <tr id="tr1">
	...
	<script type="text/javascript">
        ...
        function onOk() {
			...
			if (form.isValid() == false) {
             ...
             var vcIssueNameFull = nui.get("vcIssueNameFull").value.replace(/(^\s*)|(\s*$)/g, "");
             var vcIssueName = nui.get("vcIssueName").value.replace(/(^\s*)|(\s*$)/g, "");
             var json = nui.encode({vcIssueNameFull:vcIssueNameFull,vcIssueName:vcIssueName});
			 $.ajax({//判断是否存在重复的发行主体信息
				url:"com.cjhxfund.ats.fm.baseinfo.IssueInfoManager.checkIssurInfo.biz.ext",
	            type:'POST',
	            data:json,
	            cache:false,
	            ...
	            success:function(text){
			       ...
			       flag = text.out;
			       if(text.out==true){
			       		nui.confirm("存在类似发行主体信息，确认需要查询出来?", "系统提示", function (a){
				       		if(a=='ok'){
				      			CloseWindow(nui.get("vcIssueName").value);
				      		 }
						});
			       }else{
			       		saveData();
        ...
        function saveData(){
            //var vcIssueNameChar = query(nui.get("vcIssueName").value).toLocaleUpperCase();
            //nui.get("vcIssueNameChar").setValue(vcIssueNameChar);
            ...
            var data = form.getData(false,true);
			 ...
			 $.ajax({
				url:"com.cjhxfund.ats.fm.baseinfo.IssueInfoManager.saveIssuerInfo.biz.ext",
	            type:'POST',
	            data:json,
	            cache:false,
	            contentType:'text/json',
        			...
        			if(returnJson.exception == null){
        				nui.alert("保存成功", "系统提示", function(action){
                        		if(action == "ok" || action == "close"){
                            		CloseWindow(returnJson.vcIssueName);
                        		}
                    		});
        			}
				}    
