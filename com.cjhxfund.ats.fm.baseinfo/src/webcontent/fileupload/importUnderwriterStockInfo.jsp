...
<%
 ...
 <div style="width:100%;height:100%; "  id="dataform1" borderStyle="border:solid 1px #aaa;">
	...
	//UpSuccessBizParm[1]='zs';//user_name
		var UpSuccessBiz="com.cjhxfund.ats.fm.baseinfo.debtIssueUploadManager.underwriterStockUpload";//上传回调逻辑流全名称 逻辑流名称+逻辑构建全称，不可和回调函数名同时为空
    ...
    function startUpload(fileupload) {
		...
		fileupload.setPostParam({userid:'<%=operatorId %>'});
         ...
         var tempName = excelName.substr(excelName.indexOf("_")+1,8);//获取文件名后面的日期
         var dateName = '<%=replaceFileName %>';
         if(tempName!=dateName){
         	 form.unmask();
         	 nui.alert("请选择一个以YYYYMMDD结尾的文件导入！", "", function(){});
	         return false;
         }
        return true;
    }
