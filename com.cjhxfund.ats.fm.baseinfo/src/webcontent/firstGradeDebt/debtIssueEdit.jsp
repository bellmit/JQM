...
<!-- 								<input class="nui-combobox" textField="year" valueField="yearStr" id="lNextExerciseDate" name="lNextExerciseDate" width="170px" valueFromSelect="true"/> -->
	                    </td>
	                    <td class="form_label" align="right">
	                                                                                    特殊期限:
	                    </td>
    ...
    <script type="text/javascript">
		...
		function saveData(){
			...
			nui.ajax({
				    ...
				    success: function (text) {
					       ...
					       flag = text.out;
					       if(text.out==false){
					       		/* nui.confirm("当前债券已经存在，确认需要修改吗?", "系统提示", function (a){
						       		if(a=='ok'){
						      			addDebtIssue();
						      		 }
								}); */
		    ...
		    function addDebtIssue(){
				...
				$.ajax({
		            ...
		            success:function(text){
						...
						var returnJson = nui.decode(text);
						if(returnJson.exception == null){
	        				var code =returnJson.returnCode;
	        				if(code=="1"){
		...
		//上载时，将附件表中的id传过来，放到隐藏表单中，直接保存到数据库
		   ...
		   function SetData(data,tempVcStockType,allVcIssueAppraiseCsrc,allVcIndustry) {
            	 nui.get("vcStockType").loadList(tempVcStockType);
