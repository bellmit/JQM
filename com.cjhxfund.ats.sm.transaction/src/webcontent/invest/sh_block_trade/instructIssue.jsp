...
<body>
				...
				<div id="complete_instruction" style="padding: 5px">
					...
					<table id="instrcutInfo" class="instructTable" border="0" cellpadding="1" cellspacing="2" style="width: 100%;">
						...
						<tr>
							...
							<td>
								<input id="tradeDate" class="nui-datepicker" name="tradeDate" required="true"  style="width: 140px" 
									allowInput="false"  onvaluechanged="whetherTradeDate" 
						...
						<tr>
							...
							<td><input class="nui-textbox" 
									...
									name="enFaceAmount"  
									required="true"  
									onvalidation="enFaceAmountCheck"
									onblur="calculateNetPrice"/><span style="padding-left:3px;">万元</span></td>
						...
						<tr>
							<td align="right"><span>*</span><nobr id="seats">对手席位:</nobr></td>
							<td>
								<input id="vcRivalSeat" class="nui-textbox" style="width: 125px" name="vcRivalSeat" required="true" onvalidation="digitCheck"/>
								<input id="vcCounterpartyTrader" vtype="maxLength:64" class="nui-textbox" style="width: 125px;" name="vcCounterpartyTrader" required="true"/><!-- 对方交易员代码 -->
							</td>
						...
						<tr>
							...
							<td><input  class="nui-textbox" id="vcAgreemtCode" style="width: 125px" name="vcAgreemtCode" required="true" onvalidation="digitCheck"/></td>
	...
	<script type="text/javascript">
		...
		function SetData(data){
			...
			var vcBizType = data["vcBizType"];
			if(data.vcBizType == "3"){//固收
				nui.get("tabs").updateTab(nui.get("tabs").getTab(0),{title:"上交所固收平台"});
				nui.get("businClass").setValue("I");
				nui.get("vcBizType").setValue(vcBizType);
				nui.get("vcRivalSeat").hide();
  				nui.get("vcCounterpartyTrader").enable();
  				$("#seats").html("对方交易员:");
			}else{
				nui.get("tabs").updateTab(nui.get("tabs").getTab(0),{title:"上交所大宗平台"});
				nui.get("vcCounterpartyTrader").hide();
			}
			...
			nui.get("vcEntrustDirection").setValue(data.vcEntrustDirection);//委托方向
...
// 			var vcCounterpartyName = nui.get("vcCounterpartyId").getValue();//交易对手
	            	...
	            	if(returnJson.exception == null){
	            		if(returnJson.rtnCode == "<%=com.cjhxfund.commonUtil.Constants.ATS_SUCCESS%>"){
	            			...
	            			nui.confirm(returnJson.rtnMsg+"</br>是否继续下单？","提示", function(action){
		                		if(action == "ok") {
                                	instructionRiskIssue(instructJson);
                                }
		                	});
	            		}else if(returnJson.rtnCode == "<%=com.cjhxfund.commonUtil.Constants.ATS_ERROR%>"){
	            			var riskMsg = returnJson.riskMsg;
