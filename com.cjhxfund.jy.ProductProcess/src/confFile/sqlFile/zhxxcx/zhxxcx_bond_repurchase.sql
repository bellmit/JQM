select t.l_fund_id, --基金序号
  ...
  from (select a.l_fund_id, --基金序号
               e.vc_inter_code, --证券内码
               a.L_RIVAL_ID, --交易对手ID
               a.L_DATE, --日期
               a.l_hg_date, --回购日期
               b.vc_fund_code, --基金代码
               ...
               e.vc_report_code vc_report_code, --证券代码
               e.vc_stock_name vc_stock_name, --债券名称
               e.L_MORTAGAGE_AMOUNT / 10 l_deal_amount, --成交数量
               ...
               case
                 ...
                 when (to_number(to_char(sysdate, 'YYYYMMDD')) >
                      a.L_REDEEM_LIQUIDATE) and (a.C_REDEAL_FLAG = '0') then
                  trade.sf_date_diff(a.l_hg_date,
                               to_number(to_char(sysdate, 'YYYYMMDD')))
                 else
                  trade.sf_date_diff(a.l_Hg_Date, a.L_SETTLE_DATE)
               end l_use_days, --实际占款天数
               a.EN_DEAL_BALANCE *
               (1 +
               ...
               a.L_OPERATOR_NO, --操作员
               e.l_fund_id l_zyfund_id, --基金序号
               e.l_basecombi_id l_zybasecombi_id --组合序号
 ...
 where 1 = 1
--and vc_fund_name like '%金睿11号%'
--and c_entrust_direction = 16
--and L_DATE >= 20180328
--and L_DATE <= 20180328
