<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8" session="false" %>
	
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<%@include file="/common/js/commscripts.jsp" %>

<head>
<title>交易流水查询</title>
    <meta http-equiv="content-type" content="text/html; charset=UTF-8" />
    
</head>

<body style="width:99.6%;height:99.6%;">
<div  class="nui-tabs"  style="width:100%;height:100%;" activeIndex="0" id="tab1">
	   		</div>
	<script type="text/javascript">
    	nui.parse();
    	var queryUserType = "<%=request.getParameter("queryUserType")%>";
    	//tab页权限处理
		 nui.ajax({
			url:"com.cjhxfund.ats.fm.comm.common.queryMenuList.biz.ext",
            type:'POST',
            data:{funcCode:"ATS_TCGL_TCHZJCB_U,ATS_TCGL_FZGL_U,ATS_TCGL_BQGL_U,ATS_TCGL_TCHZJCBBY"},
            cache:false,
            contentType:'text/json',
            success:function(text){
            	var result = nui.decode(text.treeNodes);
	        	if(result){
			        var tempArray = new Array();	        		
	        		for(var k=0;k<result.length;k++){
	        			if(result[k].RES_ID=="ATS_TCGL_TCHZJCB_U"){
							tempArray[0]=result[k];
						}
						if(result[k].RES_ID=="ATS_TCGL_FZGL_U"){
							tempArray[1]=result[k];
						}
						if(result[k].RES_ID=="ATS_TCGL_BQGL_U"){
							tempArray[2]=result[k];
						}
						if(result[k].RES_ID=="ATS_TCGL_TCHZJCBBY"){
							tempArray[3]=result[k];
						}					
	        		}
	        		
		        	for(var i=0;i<tempArray.length;i++){
		        		if(tempArray[i]!=null){
					    	var actionURL = '<%=request.getContextPath()%>'+tempArray[i].FUNCACTION+"?queryUserType="+queryUserType;
					    	
							var tabs = nui.get("tab1"); 
							
					        var tab = {title: tempArray[i].FUNCNAME,url:actionURL,showCloseButton:false,refreshOnClick:"true"};
					        var tab1 = tabs.addTab(tab);    
				        }        			        
			        }
			        var tabs = nui.get("tab1");
			        var tab1 = tabs.getTab(0);
                    tabs.activeTab(tab1);	 
	            }
            }
		 });
		 
    </script>
</body>
</html>