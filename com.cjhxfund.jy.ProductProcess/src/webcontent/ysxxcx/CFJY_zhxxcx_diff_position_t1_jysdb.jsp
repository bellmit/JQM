...
<div style="margin:0px 2px 0px 2px;width:100%;height:100%;" >
	<div id="tab" activeIndex="0" onactivechanged="activechangedFun" style="width:100%;height:100%;">
	   ...
	   <div class="search-condition">
		   <div class="list">
			 <form id="form_T1JYSDB" method="post">
			 	<%-- 查询用户类型，若是投顾，则过滤产品权限 --%>
			 	<input class="nui-hidden" name="paramObject/queryUserType" value="<%=request.getParameter("queryUserType")%>">
			 	<%-- 查询类型 --%>
			 	<input class="nui-hidden" name="paramObject/queryType" value="T1JYSDB">
			 	<%-- 筛选查询类型 --%>
                ...
                <table id="table1" class="table" style="height:100%;table-layout:fixed;">
                	<tr>
                        ...
                        <td colspan="1" width="15%" align="left">
                            <input id="vCFundCode_T1JYSDB" class="nui-buttonedit" name="paramObject/vCFundCode" onbuttonclick="ButtonClickGetFundName_T1JYSDB"/>
                        	<%-- 设置默认的债券基本信息表当前业务日期 --%>
                        	<input id="preDate_T1JYSDB" class="nui-hidden" name="paramObject/preDate" />
                        </td>
                        <td class="form_label" width="60px" align="right">
							债券代码:
                        </td>
                        <td colspan="1" width="15%" align="left">
                            <input id="vCReportCode_T1JYSDB" class="nui-textbox" name="paramObject/vCReportCode"/>
                        </td>
                        <td class="form_label" width="60px" align="right">
							债券名称:
                        </td>
                        <td colspan="1" width="15%" align="left">
                            <input id="vCStockName_T1JYSDB" class="nui-textbox" name="paramObject/vCStockName"/>
                        </td>
                        <td align="right" colspan="10">
                            <input class='nui-button' plain='false' text="查询" iconCls="icon-search" onclick="search_T1JYSDB(true)"/>
                            <input class='nui-button' plain='false' text="重置" iconCls="icon-cancel"  onclick="resetDire_T1JYSDB()"/>
							<input class='nui-button' plain='false' id="export_T1JYSDB" text="导出" iconCls="icon-download" onclick="export_T1JYSDB()"/>
                        </td>
...
<script type="text/javascript">
	...
	//设置默认时间
	var preDate = "<%=preDate%>";
	nui.get("preDate_T1JYSDB").setValue(preDate);
	
	var grid_T1JYSDB = nui.get("datagrid_T1JYSDB");//交易所担保交易债券明细表
	...
	<%-- 当日T+1头寸差额开始... --%>
	//查询
	function search_T1JYSDB(flag){
		havingAlert = flag;
		nui.get("exclusivelyShowSection").setValue("1");
		search(grid_T1JYSDB, "#form_T1JYSDB");
	}
	//获取查询条件的基金名称
	function ButtonClickGetFundName_T1JYSDB(e){
        ButtonClickGetFundName(this, null);
	}
    //行双击时弹出页面展示该指令详细信息
	grid_T1JYSDB.on("rowdblclick", function (e) {
		rowdblclickFun("CFJY_tcxxcx_T1JYSDB_detail.jsp", "当日T+1头寸差额-交易所担保交易债券明细", 710, 400, e, grid_T1JYSDB);
	});
	//合并单元格
	grid_T1JYSDB.on("load", function () {
    	var length = grid_T1JYSDB.data.length;
		var datas = grid_T1JYSDB.data;
    	if(length == 0 ){
    		if(havingAlert){
    			nui.alert("没有符合筛选的数据！");
    		}
	    	...
	    	grid_T1JYSDB.mergeColumns(["L_FUND_ID","VC_FUND_CODE","VC_FUND_NAME","L_SALE_AMOUNT_COUNT"]);
    	}
    	havingAlert = false;
    });
    /** 
    ...
    //重置指令信息
	function resetDire_T1JYSDB(){
		var form = new nui.Form("form_T1JYSDB");
		form.reset();
	}
	//导出
	function export_T1JYSDB(){
	    exportSubmit("export_T1JYSDB", "form_T1JYSDB", "当日T+1头寸差额-交易所担保交易债券明细","TCXXCX");
	}
	<%-- 当日T+1头寸差额结束!!! --%>
</script>
</body>
