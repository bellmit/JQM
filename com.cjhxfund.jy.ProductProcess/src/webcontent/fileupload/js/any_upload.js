...
function fileDialogComplete(numFilesSelected, numFilesQueued) {
	...
	if(numFilesSelected!=0 && numFilesSelected==numFilesQueued){
	 	//msgid = window.parent.commonLoading("正在上传......","上传");
		this.startUpload();
...
function uploadSuccess(file, serverData) {
	try {
		if(msgid!=null&&msgid!=""){
			window.parent.commonHideMessageBox(msgid);
		}
		var fileid = trim(serverData.toString());
...
function addReadyFileInfo(fileid,fileName,message,status,size1){
	...
	col5.style.width="80";
	//新增上传文件
	var newRow = { sysid:fileid,name:fileName,message:message,status:status,attachSize:attachSize};
