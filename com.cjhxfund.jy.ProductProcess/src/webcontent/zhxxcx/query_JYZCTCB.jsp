	...
	<script type="text/javascript">
    	...
    	//tab页权限处理
		 nui.ajax({
            ...
            type:'POST',
            data:{funcCode:"ATS_JSGL_HKZLZCB_S,ATS_JSGL_HKZLZCB_BY"},
            cache:false,
            ...
            success:function(text){
	        	...
	        	if(result){
	        		...
	        		for(var k=0;k<result.length;k++){
	        			if(result[k].RES_ID=="ATS_JSGL_HKZLZCB_S"){
							tempArray[0]=result[k];
						}
						if(result[k].RES_ID=="ATS_JSGL_HKZLZCB_BY"){
		        	...
		        	for(var i=0;i<tempArray.length;i++){
		        		if(tempArray[i]!=null){
					    	var actionURL = '<%=request.getContextPath()%>'+tempArray[i].FUNCACTION+"?page=1&queryUserType="+queryUserType;
