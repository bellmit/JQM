						...
						<table id="table1" class="table" style="height:100%;table-layout:fixed;">
							<tr>
								...
								<td colspan="1" width="20%">
									<input id="busiDateEnd_HSFAGZB" class="nui-datepicker" name="paramObject/busiDateEnd"/>
								</td>
								<td class="form_label" width="7%">
									基金名称:
								</td>
								<td colspan="1" width="35%">
									<input id="vCFundCode_HSFAGZB" class="nui-buttonedit" width="250px" name="paramObject/vCFundCode" onbuttonclick="ButtonClickGetFundName_HSFAGZB"/>
								</td>
...
<script type="text/javascript">
	...
	function search_HSFAGZB(){
		...
		for(var i=0; i<fundNameArr.length; i++){
			...
			//设置tab属性
			var tab = {title:fundCodeArr[i]+"-"+fundNameArr[i], url:"<%=request.getContextPath() %>/ProductProcess/zhxxcx/CFJY_zhxxcx_hsfagzb_detail.jsp?vCFundCode="+fundCodeArr[i]+"&busiDateEnd="+busiDateEnd+"&queryUserType="+queryUserType+"&vCFundName="+vCFundName, showCloseButton: true };
			//设置tab
			tabs.addTab(tab);
			//当i=0时模拟点击事件
 			if(i == 0){
