...
public class CashFlowUtil {
	...
	*/
	...
	public List<DataObject> queryPositionForecast(DataObject paramObject, DataObject page,String type){
		...
		int endDate = 0;
		if(paramObject.getString("endDate")!=null){			
			endDate = Integer.parseInt(paramObject.getString("endDate"));//结束日期（查询条件传入）
		}
		...
		//根据基金代码列表查询基金编号列表
		if(vcFundCodes !=null && vcFundCodes !="" && endDate!=0){
			//将字符串转成数组
			...
			try{
            	...
            	if("1".equals(basePostion.getCode())){
            		...
            		for(int i=0;i<lengthPosition;i++){
        				...
        				/**港股业务**/
        				resultData.set("stockBuyBalHgt", postionList.get(i).getStockBuyBalHgt()==null?0.00:postionList.get(i).getStockBuyBalHgt());//沪港通股票买入金额
        				resultData.set("stockSaleBalHgt", postionList.get(i).getStockSaleBalHgt()==null?0.00:postionList.get(i).getStockSaleBalHgt());//沪港通股票卖出金额
        				resultData.set("riskBalHgt", postionList.get(i).getRiskBalHgt()==null?0.00:postionList.get(i).getRiskBalHgt());//沪港通风控资金
        				resultData.set("stockBuyBalSgt", postionList.get(i).getStockBuyBalSgt()==null?0.00:postionList.get(i).getStockBuyBalSgt());//深港通证券买入金额
        				resultData.set("stockSaleBalSgt", postionList.get(i).getStockSaleBalSgt()==null?0.00:postionList.get(i).getStockSaleBalSgt());//深港通证券卖出金额
        				resultData.set("riskBalSgt", postionList.get(i).getRiskBalSgt()==null?0.00:postionList.get(i).getRiskBalSgt());//深港通风控资金
        				/**TA业务**/
        				resultData.set("applyTaBal", postionList.get(i).getApplyTaBal()==null?0.00:postionList.get(i).getApplyTaBal());//TA申购
        				resultData.set("redeemTaBal", postionList.get(i).getRedeemTaBal()==null?0.00:postionList.get(i).getRedeemTaBal());//TA赎回
        				resultData.set("dividendTaBal", postionList.get(i).getDividendTaBal()==null?0.00:postionList.get(i).getDividendTaBal());//TA分红
        				/**分销业务**/
        				...
        				resultData.set("vatBal", postionList.get(i).getVatBal()==null?0.00:postionList.get(i).getVatBal());//增值税
        				/**手工业务查询逻辑待实现**/
