        	...
        	if(returnJson.exception == null){
        		if(returnJson.rtnCode == ATS_SUCCESS){
        			...
        			nui.confirm(returnJson.rtnMsg+"</br>是否继续下单？","提示", function(action){
                		if(action == "ok") {
                			formalInstructionEntry(param,close);
                        }
                	});
...
//正式修改指令/建议信息
function formalInstructionEntry(instructJson,close){
			...
			if(returnJson.exception == null){
				if(returnJson.rtnCode == ATS_SUCCESS){
        			...
        			if(riskMsg && riskMsg.isAgainstRisk){
        				...
        				nui.alert("投资指令/建议下达失败！</br>"+returnJson.rtnMsg,"提示");
        			}
	            } else{
	            	nui.alert("投资指令/建议下达成功！</br>"+returnJson.rtnMsg,"提示",function(){
	            		formalInstructionEntryFinish();
	                });
	            }
			}else{
...
$(function(){
	...
	bondsGrid.on("load",function(){
		showDetail('bondsGrid');
	});
});
