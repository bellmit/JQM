...
function combiDireParam(){
    ...
    if ((nui.get("vcEntrustDirection").getValue() == "5")  && bondData.length < 1) {
        nui.alert("您没有选择任何可质押的债券。", "警告");
        return false;
    }
    var zeroBonds = false;
        	...
        	if(returnJson.exception == null){
        		if(returnJson.rtnCode == ATS_SUCCESS){
        			...
        			if(returnJson.rtnMsg){
        				riskMsg = "</br>【该投资指令/建议触发以下风控：</br>"+returnJson.rtnMsg+"】";
        			}
        			nui.alert(successMsg+riskMsg,"提示",function(){
        				if(isUpdateDire){
        					CloseOwnerWindow();
        				}
        				
                    });
        		}else if(returnJson.rtnCode == ATS_ORDER_USABLE_AMOUNT_INSUFFICIENT){//可用不足,继续下单先校验风控
        			nui.confirm(returnJson.rtnMsg+"</br>是否继续下单？","提示", function(action){
                		if(action == "ok") {
                        	instructionRiskIssue(param);
                        }
                	});
        		}else if(returnJson.rtnCode == "203"){//风控校验失败
        			nui.confirm(returnJson.rtnMsg+"</br>是否继续下单？","提示", function(action){
                		if(action == "ok") {
        		...
        		}else if(returnJson.rtnCode == ATS_ORDER_NO_RCV_FR_O32){
        			nui.alert(successMsg+returnJson.rtnMsg,"提示",function(){
        				if(isUpdateDire){
        				CloseOwnerWindow();
        				}
                    });
...
function instructionRiskIssue(instructJson){
	var successMsg = "投资指令/建议下达成功！";
	if(isUpdateDire){//修改指令
    	successMsg = "投资指令/建议修改成功！";
    }
	...
	nui.ajax({
		url: "com.cjhxfund.ats.sm.comm.InstructionManager.checkInstructRisk.biz.ext",
		type: 'POST',
		data: instructJson,
		cache: false,
		contentType: 'text/json',
	    success: function (text) {
	    	...
	    	if(returnJson.exception == null){
	    		if(returnJson.rtnCode == ATS_SUCCESS){
	    			...
	    			nui.alert(successMsg+"</br>"+returnJson.rtnMsg,"提示",function(){
	    				if(isUpdateDire){
	    				CloseOwnerWindow();
	    				}
	                });
...
function formalInstructionEntry(instructJson){
	...
	var successMsg = "投资指令/建议下达成功！";
	var errorMsg = "投资指令/建议下达失败！";
	if(isUpdateDire){//修改指令
    	successMsg = "投资指令/建议修改成功！";
    	errorMsg = "投资指令/建议修改失败！";
	    ...
	    success: function (e) {
			...
			if(returnJson.exception == null){
				if(returnJson.rtnCode == ATS_SUCCESS){
	            	...
	            	nui.alert(errorMsg+"</br>"+returnJson.rtnMsg,"提示");
	            } else{
	            	nui.alert(successMsg+"</br>"+returnJson.rtnMsg,"提示",function(){
	            		if(isUpdateDire){
	            		CloseOwnerWindow();
...
function saveInstruction(instructJson){
	var url = "com.cjhxfund.ats.sm.comm.InstructionManager.insertInstruct.biz.ext";
    ...
    if(isUpdateDire){//修改指令
    	...
    	errorMsg = "指令修改失败";
        url = "com.cjhxfund.ats.sm.comm.InstructionManager.insertAndUpdateInstruct.biz.ext";
        //instructJson["srcInstruct"] = text.resultSrc;
    ...
    nui.ajax({
        ...
        success: function (e) {
        	nui.hideMessageBox(a);
            if(e.exception == null){
                if (e.rtnCode == ATS_ERROR || e.rtnCode == ATS_ORDER_USABLE_AMOUNT_INSUFFICIENT){
                	if(e.rtnMsg != null){
                		nui.alert(errorMsg+","+e.rtnMsg,"提示");
                	}else{
