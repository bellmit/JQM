...
var calFirstMaturityRe = function () {
    ...
    if (nui.get("vcSettleSpeed").getValue() == "1") {//判断清算速度
    	var settleSpeed = 1;
    	if(isNotTradeDate(lTradeDateValue,"00")){//判断交易日是否为交易日
			settleSpeed = 0;
		}
        nui.ajax({
            ...
            success: function (resp) {
                ...
                nui.get("lFirstSettleDate").setValue(resp.maturitySettleDate);//首次结算日期
                var temp = DateUtil.numStrToDateStr(lFirstSettleDate);
                calMaturityDate(temp, lRepoDays);
              //限制到期结算日期
                limitTradeDate(temp,"lMaturitySettleDate");//到期结算日期:
            }
        });
    } else {
...
var calMaturityDate = function (firstSettleDateStr, repurchDays) {
   if (!repurchDays || repurchDays == "") {
    ...
    nui.ajax({
        ...
        contentType: 'text/json',
        success: function (resp) {
            nui.get("lMaturitySettleDate").setValue(resp.workDay);//到期结算日期
            //计算占款天数
            calPeriods();
            //计算利息和到期结算金额
            calEmFaceAmount();
...
function calPeriods(){
	...
	var lMaturitySettleDate = nui.get('lMaturitySettleDate').getValue();//到期结算日期
	...
	nui.get("lContractPeriods").setValue(parseInt(value));//占款天数
	calEmFaceAmount();
}
//计算利息和到期结算金额
function calEmFaceAmount() {
    ...
    nui.get("enSettleAmount").setValue(amount);//到期结算金额
};
//报价方式
...
//    fullDire["lRepoDays"] = days;
    ...
    directionParam["cOverdraft"] = cOverdraft;//是否可以透支
    directionParam["vcMarket"] = "5"; //市场编号
    return {instructParameter: directionParam, bonds: bondData};	// inquiryResultType：1=回购，2=买卖
}
