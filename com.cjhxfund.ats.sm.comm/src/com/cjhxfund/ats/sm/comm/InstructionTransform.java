...
import com.cjhxfund.commonUtil.ArithUtil;
	...
	public static Map<Object, Object> getRiskControlMsgTransform(DataObject obj, String clordid, String instructionno){
		...
		if(bonds!=null && bonds.length > 0){//回购业务处理
			...
			for(int i=0 ; i < bonds.length; i++){//组合质押券信息
				...
				Double conversionRatio = ArithUtil.div(StrUtil.delThousandthEmptyTo0(bond.getString("enMortagageRatio")), 100);//质押比率
				int orderqty = (int) (bondAmount*conversionRatio/100);
				//组合|投资类型|质押券|质押比例|质押数量
				if("O".equals(obj.getString("businClass"))){
					checkMemo += obj.getString("vcCombiCode")+"|"+obj.getString("vcInvestType")+"|"+bond.get("vcReportCode")+"|"+orderqty;
				}else{
			...
			if("O".equals(obj.getString("businClass"))){//买断式回购
				bean.setString("vcPrice", bonds[0].getString("enNetPriceInit"));//44-指令价格
				sendMsg.setString("vcPrice", bonds[0].getString("enNetPriceInit"));//首期净价价格
					System.out.println(String.valueOf(StrUtil.delThousandthEmptyTo0(obj.getString("enFullPriceAmount"))*10000/100));
					bean.setString("vcOrderqty", String.valueOf(StrUtil.delThousandthEmptyTo0(obj.getString("enFullPriceAmount"))*10000/100));//38-指令数量
				...
				bean.setString("vcCashorderqty", String.valueOf(StrUtil.delThousandthEmptyTo0(obj.getString("enFullPriceAmount"))));//152-指令金额
				sendMsg.setString("vcCashorderqty", String.valueOf(StrUtil.delThousandthEmptyTo0(obj.getString("enFullPriceAmount"))));//指令金额
				
				bean.setString("vcBondYield", String.valueOf(StrUtil.delThousandthEmptyTo0(bonds[0].getString("enFirstYield"))/100));
				bean.setString("vcBondYield2", String.valueOf(StrUtil.delThousandthEmptyTo0(bonds[0].getString("enMaturityYield"))/100));
				bean.setString("vcRealBondInterest",bonds[0].getString("enMaturitySettleInterest"));
				bean.setString("vcStopPrice",bonds[0].getString("enNetPriceFinal"));
				
			} else {//其他回购
				bean.setString("vcPrice", vcPrice);//44-指令价格
				sendMsg.setString("vcPrice", vcPrice);//44-指令价格
					...
					sendMsg.setString("vcOrderqty", new BigDecimal((new Double(StrUtil.delThousandthEmptyTo0(obj.getString("enFaceAmount"))).longValue())*10000/100).toString());//指令数量(张)，机器猫前端页面单位为万元，所以需要乘以10000(元)，又因100元/张，所以再需要除以100.
				...
				sendMsg.setString("vcCashorderqty", new BigDecimal(StrUtil.delThousandthEmptyTo0(obj.getString("enFaceAmount"))*10000).toString());//指令金额
			}
		}else{//买卖业务处理            ;                                     	 
		...
		if(( "0".equals(obj.getString("startApi")) && "1".equals(obj.getString("smStartApi")) && ( "1".equals(obj.getString("callRiskType")) || "3".equals(obj.getString("callRiskType")) ) )){
			...
			//上交所固收平台、上交所固收平台、深交所综合协议平台：只调用本地风控
			if("2".equals(obj.getString("vcBizType")) || "3".equals(obj.getString("vcBizType")) || "4".equals(obj.getString("vcBizType"))){
		...
		//sendMsg.setString("vcOrdtype", obj.getString("ordtype")); 
		if("2".equals(obj.getString("businClass"))){
			...
			if(((Object[])obj.get("bonds")).length>0){
				...
				if(bonds!=null && bonds.length > 0){//回购业务处理
					...
					for(int i=0; i<bonds.length; i++){
						...
						bean.setString("vcPledgeratio", String.valueOf(ArithUtil.div(StrUtil.delThousandthEmptyTo0(bond.getString("enMortagageRatio")),100)));//质押比例，抵押的比例90％填0.9，机器猫前台单位为%，所以需除以100.
