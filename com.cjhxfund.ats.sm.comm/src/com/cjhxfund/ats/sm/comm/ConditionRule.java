...
public class ConditionRule {
	...
	/**/
	public static String ZHFWPT_ENABLE_FIX = "ZHFWPT_ENABLE_FIX";
...
//						}
	...
	public static Boolean inquiryResultInvestManageAutoFinished(String createUserID,String productCode, String vcCombiCode, String vcInstructType) throws ParseException{
		if("true".equals(checkInvestManager(createUserID,productCode))){//流程创建者为投资经理
			...
			if("1".equals(ParamConfig.getValue(ZHFWPT_ENABLE_FIX)) || "1".equals(ParamConfig.getValue(AUTO_INVEST_MANAGER_PARAM_KEY)) || "2".equals(vcInstructType)){
				//投资经理确认环节模式为手工确认或者指令为询价指令或未启用Fix对接
				return false;
			}else{
				return true;
			}
