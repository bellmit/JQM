...
var ua = navigator.userAgent.toLowerCase(), check = function (a) {
	return a.test(ua)
}, DOC = document, isStrict = document.compatMode == "CSS1Compat", version = function (c, b) {
	var a;
...
mini.Popup_prototype = {
	...
	__OnBodyMouseDown: function (a) {
		...
		if (this.within(a) || (this.popupEl && mini.isAncestor(this.popupEl, a.target))) {}
		else {
			this.doHide(a)
		}
	},
	_tryHide: function (b) {
		if (mini.isAncestor(this.el, b.target) || (this.popupEl && mini.isAncestor(this.popupEl, b.target))) {}
		else {
...
mini.extend(mini.TextBox, mini.ValidatorBase, {
	...
	__OnInputKeyDown: function (c) {
		...
		if (c.keyCode == 27 || c.keyCode == 13 || c.keyCode == 9) {
			if (this._InputType == "textarea" && c.keyCode == 13) {}
			else {
...
mini.extend(mini.Calendar, mini.Control, {
	...
	_CreateView: function (m, d, a) {
		...
		if (this.showHeader) {
			...
			n += '<span class="mini-calendar-title">' + mini.formatDate(m, this.format);
			 + "</span>";
			n += "</div></td></tr>"
		}
...
mini.extend(mini.TextBoxList, mini.ValidatorBase, {
	...
	__OnClick: function (b) {
		...
		var a = this.getItemByEvent(b);
		if (!a) {
			if (mini.findParent(b.target, "mini-textboxlist-input")) {}
			else {
				this.showInput()
			}
			return
...
mini.extend(mini.Splitter, mini.Control, {
	...
	__OnMouseDown: function (c) {
		...
		if (mini.isAncestor(this._handlerEl, a)) {
			if (a.className == "mini-splitter-pane1-button" || a.className == "mini-splitter-pane2-button") {}
			else {
				var b = this._getDrag();
				b.start(c)
			}
...
mini.copyTo(mini.Layout.prototype, {
	...
	__OnDocMouseDown: function (d) {
		...
		for (var b = 0, a = this.regions.length; b < a; b++) {
			...
			if (mini.isAncestor(c._el, d.target) || mini.isAncestor(c._proxy, d.target) || d.target.location) {}
			else {
			...
			success: function (x, q, y) {
				...
				if (mini.isNumber(w) && w != 0 || w === false) {
					...
					if (k) {
						...
						if (l.getSelected() == null && l.selectOnLoad && l.getDataView().length > 0) {
							l.select(0)
						}
						l.fire("load", v);
						if (n) {
