...
<script type="text/javascript">
	...
	grid.on("drawcell", function (e) {
		...
		if (column.name == "action") {
			...
			if(row.VC_PROCESS_TYPE&&(row.VC_PROCESS_TYPE=='11'||e.row.VC_PROCESS_TYPE=='13'||e.row.VC_PROCESS_TYPE=='14'||e.row.VC_PROCESS_TYPE=='15')){
	...
	grid.on("rowdblclick", function (e) {
		...
		if(e.row.VC_PROCESS_TYPE&&(e.row.VC_PROCESS_TYPE=='11'||e.row.VC_PROCESS_TYPE=='12'||e.row.VC_PROCESS_TYPE=='13'||e.row.VC_PROCESS_TYPE=='14'||e.row.VC_PROCESS_TYPE=='15')){
			var url = "/com.cjhxfund.fpm.bpsExpend.getTaskInfo.flow?workItemID="+e.row.WORKITEMID;
			wfOpenWin(url);
		} else {
	...
	function batchConfirm(){
	   ...
	   if(rcds.length == 0){
		...
		}else if(type == "11"||type == "12"){//如果风控流程
			batchCheck();
		}else{
	...
	function batchCheck(){
		...
		for(var i=0;i<rows.length;i++){
			var row = rows[i];
			var data = {};
			data = {lBizId:row.L_BIZ_ID,processInstId:row.L_PROCESS_INST_ID,activityInstName:row.ACTIVITYINSTNAME,userId:operatorId}
			riskDatas[i] = data;
		}
