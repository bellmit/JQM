...
<%
  ...
  String agentAccessURL = ParamConfig.getValue("ZHFWPT_AGENT_ACCESS_URL");//获取代理服务器URL
  String serverType = ParamConfig.getValue("ZHFWPT_SERVER_TYPE");//获取URL类型：0-获取本地路径（测试环境）；1-获取设置在数据中的代理路径（生产环境）；

  String mServerUrl = "";
  String mHttpUrl = "";

  String applyName = mHttpUrlName.substring(0,mHttpUrlName.lastIndexOf(mScriptName));    //获取项目应用名称

  //判断当前是获取本地路径还是代理路径
  if(serverType.equalsIgnoreCase("0") && serverType != null){
  	mServerUrl = "http://" + request.getServerName() + ":" + request.getServerPort() + applyName + "/" + mServerName;//取得OfficeServer文件的完整URL
  ...
  if (mTemplate==null){
    ...
    FileAdd =  request.getParameter("filePath");
    
    System.out.println(mFileName);System.out.println(FileAdd);
  ...
  if (mFileType.equalsIgnoreCase(".doc") || mFileType.equalsIgnoreCase(".wps")||mFileType.equalsIgnoreCase(".xml")){
...
function Load(){
  try{
    ...
    webform.WebOffice.WebUrl="<%=mServerUrl%>?Filepath=<%=URLEncoder.encode(FileAdd,"utf-8") %>&FileType=1";      //WebUrl:系统服务器路径，与服务器文件交互操作，如保存、打开文档，重要文件
    ...
    webform.WebOffice.Compatible=true; 						//true表示为兼容模式
...
function EditStatus(){
	try{
		alert(webform.WebOffice.EditStatus);
		if(!webform.WebOffice.EditStatus){
			alert("文档不允许编辑！");
		}
	}catch (e) {
		alert(e.description);
...
<form name="webform" method="post" action="iWebOffice/DocumentSave.jsp"  onsubmit="return SaveDocument();">  <!--保存iWebOffice后提交表单信息-->
    ...
    <input type="hidden" name="HTMLPath" value="<%=mHTMLPath%>">
  	<input type="hidden"  value="保存文档" id="submit" style="width: 80px;height: 30px;" />
