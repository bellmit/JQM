    ...
    try {
       ...
       if(fileName.indexOf("[用印]") == -1){
         if(fileName.indexOf(".docx") != -1 || fileName.indexOf(".doc") != -1){
       	    ...
       	    fileName = "[用印]"+fileName.replace(".xlsx", ".xls"); 
       	 }	
       }else{
         if(fileName.indexOf(".docx") != -1 || fileName.indexOf(".doc") != -1){
       ...
       String  filePath = mFilePath.substring(0,mFilePath.lastIndexOf(File.separator) +1) + fileName;
	    ...
	    if(attachName.indexOf("[用印]") == -1 && obj.getInt("vcVersionNo") != 2){  
	        ...
	        //判断文件类型，并转换文件格式
	        if(attachName.indexOf(".xlsx") != -1){
	         attachName = attachName.replace(".xlsx", ".xls");
	        }else if(attachName.indexOf(".docx") != -1){
	         attachName = attachName.replace(".docx", ".doc");
	        }
        ...
        if (request.getMethod().equalsIgnoreCase("POST")) {
          ...
          if (MsgObj.GetMsgByName("DBSTEP").equalsIgnoreCase("DBSTEP")) {         //判断是否是合法的信息包，或者数据包信息是否完整
            ...
            if (mOption.equalsIgnoreCase("LOADFILE")) {                           //下面的代码为打开服务器数据库里的文件
              ...
              mFilePath = request.getParameter("Filepath");                       //获取路径
			  mInfo = request.getParameter("FileType");
			  System.out.println("mInfo  ==========" + mInfo);
              ...
              if(mInfo.equals("2")){
              		if(fileEditorSave()){
                		...
                		MsgObj.MsgError("");
                		MsgObj.MsgFileLoad(mFilePath);     //保存后刷新文档   
              		}else{
