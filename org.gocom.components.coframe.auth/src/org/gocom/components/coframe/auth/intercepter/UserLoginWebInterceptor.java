...
public class UserLoginWebInterceptor implements IWebInterceptor {
	...
	public void doIntercept(HttpServletRequest request,
		...
		if (HttpUrlHelper.isIn(servletPath, UserLoginCheckedFilter.getIncludeUrls())) {
			...
			if(!"/coframe/auth/login/logout.jsp".equals(servletPath) && !"coframe/auth/skin1/index.jsp".equals(servletPath)){
				request.setAttribute("loginMsg", "会话超时，请重新登录！");//杨敏嵌入 2016-11-14，嵌入session超时提示变量设置，在login.jsp显示。
			}
