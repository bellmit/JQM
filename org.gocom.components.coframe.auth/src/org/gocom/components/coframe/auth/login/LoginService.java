...
public class LoginService {
	...
	public DataObject[] getUserMenuList(){
		...
		for (MenuTreeNode node : menuTree) {
			getUserMenu(menuList,(LevelMenuTreeNode)node,null,map);
		}
		return menuList.toArray(new DataObject[menuList.size()]);
	}
	
	private void getUserMenu(List<DataObject> menuList,LevelMenuTreeNode node,String pid,Map map){
		boolean isLeaf = node.getChildrenMenuTreeNodeList()==null;
		DataObject menu = DataObjectHelper.createDataObject(null);
		String newMenu= map.containsKey(node.getMenuPrimeKey())==true?"<span class='newstyle'>New</span>":"";
		menu.set("text"        ,node.getMenuName()+newMenu);
		...
		if (!isLeaf&&node.getChildrenMenuTreeNodeList().size()>0){
			...
			for (MenuTreeNode menuTreeNode : childMenuTree) {
				getUserMenu(menuList,(LevelMenuTreeNode)menuTreeNode,node.getMenuPrimeKey(),map);
			}
		}
	}
	
	// 新增获取新菜单
		private  Map getNewMenu(){
		  
			Map map =new HashMap<String, String>();
			Object[]  newMenus= null;
			String nameSqlId="org.gocom.components.coframe.framework.application.impl.getNewMenusql.getNewMenu";
			newMenus = com.eos.foundation.database.DatabaseExt.queryByNamedSql("default", nameSqlId, null);
			
			for (Object menu : newMenus) {
				 String MENUID= ((DataObject) menu).getString("MENUID");
				 String ISNEW= ((DataObject) menu).getString("ISNEW");
				 map.put(MENUID, ISNEW);
			}
			return map;
