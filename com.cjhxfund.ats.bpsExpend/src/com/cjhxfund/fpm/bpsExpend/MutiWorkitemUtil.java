...
public class MutiWorkitemUtil {
	@Bizlet("比较数字大小")
	public boolean compareAgree(double allNum,double agreeNum,String termNumber){
		
		//得到同意人数占总人数的比例
		double theNum  = agreeNum/allNum;
		
		if(termNumber.equals("1")|| theNum == 1){ //如果条件为全部人通过，则直接返回;或最后一个同意
			return false;
		}else{
			String[] array = termNumber.split("/");
			if(array.length>1){
				double termNum1 = Double.parseDouble(array[0]); //将数据库条件的值类型转换
				double termNum2 = Double.parseDouble(array[1]);
				double termNum =termNum1/termNum2;
				if(theNum>termNum){  //如果比例满足条件
					return true;
				}else{
					return false;
				}
	...
	@Bizlet("比较不同意数字大小")
	public boolean compareDisAgree(double allNum,double disAgreeNum,String termNumber){
		//得到同意人数占总人数的比例
		double theNum  = disAgreeNum/allNum;
		if(termNumber.equals("1")){ //如果条件为全部人通过，则直接返回
			if(theNum>0){	//有一个人不同意，则回退
				return true;
			}
			return false;
		}else{
			...
			if(array.length>1){
				double termNum1 = Double.parseDouble(array[0]); //将数据库条件的值类型转换
				double termNum2 = Double.parseDouble(array[1]);
				double distermNum = termNum2-termNum1;
				double termNum =distermNum/termNum2;
